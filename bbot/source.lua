...
if
    isfolder(folderName)
    and isfolder(folderName .. '/cache')
    and isfolder(folderName .. '/cache/votekick data')
    and isfile(folderName .. '/cache/votekick data/' .. fileName)
    and readfile(folderName .. '/cache/votekick data/' .. fileName)
        ~= userName
then
    local hostName = readfile(
        'Phantom Forces Cheat/cache/votekick data/'
            .. tostring(game.JobId)
            .. '.txt'
    )
    ...
    for _, func in getgc(false) do
        if
            type(func) == 'function'
            and islclosure(func)
            and debug.getinfo(func).name == 'require'
            and string.find(debug.getinfo(func).source, 'ClientLoader')
        then
            require_module = func
            modules = {}
    ...
    game:GetService('RunService'):Set3dRenderingEnabled(false) -- increases performance, might not work on some executors
    ...
    function clientEvents.console(message)
        task.spawn(function()
            if string.find(message, 'has initiated a votekick on') then
                ...
                local victim = string.split(
                    string.split(message, 'initiated a votekick on ')[2],
                    ' for '
                )[1]

                repeat
                    task.wait()
                until isKickInProgress()
    ...
    repeat
        repeat
            task.wait()
        until not roundSystem.roundLock
        charInterface.spawn()
        repeat
            task.wait()
        until charInterface.isAlive()
            and charInterface.getCharacterObject()
            and charInterface.getCharacterObject():canJump()
...
LPH_NO_VIRTUALIZE(function()
    workspace:FindFirstChild(
        'Mainstreamed is cool'
    )
    do -- Bad Drawing Library
        local drawing = {}
        local cache = {
            updates = {},
            instances = {},
        ...
        local defaults =
            { --  bbot ui was originally made with synapse X' drawing api
                Square = {
                    Position = nv,
                    Size = nv,
                    Thickness = 1,
        ...
        local function destroyEntity(entity)
            if
                entity._data.shape == 'Circle'
                or entity._data.shape == 'Quad'
            then
        ...
        local function getPointOrder(a, b, c)
            local p0, p1, p2
            local d1, d2, d3 =
                (a - b).Magnitude, (c - b).Magnitude, (a - c).Magnitude
        ...
        local function renderTriangle(leftSide, rightSide, p0, p1, p2) -- creates any triangle image by turning random triangles into 2 right triangles and using right triangle images
            ...
            if
                horizontal_dot > 0 and vertical_dot < 0
                or horizontal_dot < 0 and vertical_dot > 0
            then
                if d1.X ~= 0 then
                    left = rightSide
                    right = leftSide
            ...
            for shapeIndex, updateList in cache.updates do
                ...
                elseif shape.shape == 'Text' then
                    ...
                    if updateList.Position then
                        label.Position = UDim2.new(
                            0,
                            updateList.Position.X,
                            0,
                            updateList.Position.Y + 2
                        )
                    end

                    if updateList.Center ~= nil then
                        label.AutomaticSize = updateList.Center
                                and Enum.AutomaticSize.Y
                            or Enum.AutomaticSize.XY
                    end

                    if updateList.Outline ~= nil then
                        label.TextStrokeTransparency = updateList.Outline and 0
                            or 1
                    end
                ...
                elseif shape.shape == 'Square' then
                    ...
                    then
                        ...
                        local boxPos = fixvec(
                            v2.new(
                                position.X - thicknessOffset,
                                position.Y - thicknessOffset
                            )
                        )
                        drawings.box.Position =
                            UDim2.new(0, boxPos.X, 0, boxPos.Y)
                        drawings.box.Size =
                            UDim2.new(0, size.X + thick, 0, size.Y + thick)
                        drawings.line1.Position = drawings.box.Position
                        drawings.line2.Position = UDim2.new(
                            0,
                            boxPos.X + size.X - 1,
                            0,
                            boxPos.Y + realThick
                        )
                        drawings.line3.Position =
                            UDim2.new(0, boxPos.X, 0, boxPos.Y + size.Y - 1)
                        drawings.line4.Position =
                            UDim2.new(0, boxPos.X, 0, boxPos.Y + realThick)
                        drawings.line2.Size =
                            UDim2.new(0, realThick, 0, size.Y - realThick - 1)
                        drawings.line1.Size =
                            UDim2.new(0, size.X + thick, 0, realThick)
                        drawings.line4.Size =
                            UDim2.new(0, realThick, 0, size.Y - realThick - 1)
                        drawings.line3.Size =
                            UDim2.new(0, size.X + thick, 0, realThick)
                    ...
                    if updateList.Transparency then
                        ...
                        drawings.line1.Transparency = 1
                            - updateList.Transparency
                        drawings.line2.Transparency = 1
                            - updateList.Transparency
                        drawings.line3.Transparency = 1
                            - updateList.Transparency
                        drawings.line4.Transparency = 1
                            - updateList.Transparency
                    end
                ...
                elseif shape.shape == 'Circle' then
                    ...
                    if
                        updateList.Position
                        or updateList.Thickness
                        or updateList.Radius
                        or updateList.NumSides
                    then
                        local position = shape.Position
                        ...
                        for lineIndex = 1, num do
                            ...
                            local o0 =
                                v2.new(math.cos(origin), math.sin(origin))
                            local o1 =
                                v2.new(math.cos(target), math.sin(target))
                            ...
                            line.Position = UDim2.new(0, middle.X, 0, middle.Y) -- middle
                            line.Rotation =
                                math.deg(math.atan(offset.Y / offset.X))
                            line.Size = UDim2.new(
                                0,
                                math.floor(distance + 0.5),
                                0,
                            ...
                            local rotation = math.deg(
                                (lineIndex - 0.5) * interval - (math.pi * 0.5)
                            )
                            local leftPosition = (lineIndex - 1) * interval
                            leftPosition = position
                                + v2.new(
                                        math.cos(leftPosition),
                                        math.sin(leftPosition)
                                    )
                                    * size
                                    * 0.5
                            left.Position =
                                UDim2.new(0, leftPosition.X, 0, leftPosition.Y)
                            left.Size = UDim2.new(0, distance * 0.5, 0, newSize)
                            left.Rotation = rotation
                            local rightPosition = (lineIndex - 0) * interval
                            rightPosition = position
                                + v2.new(
                                        math.cos(rightPosition),
                                        math.sin(rightPosition)
                                    )
                                    * size
                                    * 0.5
                            ...
                            right.Size =
                                UDim2.new(0, distance * 0.5, 0, newSize)
                            right.Rotation = rotation
                        end
                    end
                ...
                elseif shape.shape == 'Triangle' then
                    ...
                    if
                        updateList.PointA
                        or updateList.PointB
                        or updateList.PointC
                        or updateList.Thickness
                    then
                        ...
                        if a and b and c and a ~= b and a ~= c and b ~= c then
                            ...
                            line2.Size = UDim2.new(
                                0,
                                math.floor(d2.Magnitude + 0.5),
                                0,
                                math.abs(shape.Thickness)
                            )
                            ...
                            line3.Size = UDim2.new(
                                0,
                                math.floor(d3.Magnitude + 0.5),
                                0,
                                math.abs(shape.Thickness)
                            )
                    ...
                    if updateList.Transparency then
                        drawings.left.ImageTransparency = 1
                            - updateList.Transparency
                        drawings.right.ImageTransparency = 1
                            - updateList.Transparency
                        ...
                        then
                            ...
                            if not intersects then -- if not intersecting then gets the point with the biggest angle, 2 scalene triangles will share that point and the opposite point
                                ...
                                for lineIndex = 1, 4 do
                                    local o0 =
                                        lines[(lineIndex == 1 and 4) or lineIndex - 1][1]
                                    local o1, t1 =
                                        table.unpack(lines[lineIndex])
                                    local supangle = (o0 - o1).Unit:Dot(
                                        (t1 - o1).Unit
                                    )
                                    local angle
                            ...
                            if intersects then
                                ...
                                local lt1, rt1 =
                                    table.unpack(drawings.triangles[1])
                                local lt2, rt2 =
                                    table.unpack(drawings.triangles[2])
                                local a1, b1, c1 =
                                    getPointOrder(intersection, l1[1], l1[2])
                                local a2, b2, c2 =
                                    getPointOrder(intersection, l2[1], l2[2])
                                renderTriangle(lt1, rt1, a1, b1, c1)
                                renderTriangle(lt2, rt2, a2, b2, c2)
                            else
                                local l0 =
                                    lines[(obtuse < 3 and obtuse + 2) or obtuse - 2]
                                ...
                                local lt2, rt2 =
                                    table.unpack(drawings.triangles[2])
                                local a1, b1, c1 =
                                    getPointOrder(l1[1], l1[2], l0[1])
                                local a2, b2, c2 =
                                    getPointOrder(l1[1], l0[2], l0[1])
                                renderTriangle(lt1, rt1, a1, b1, c1)
                                renderTriangle(lt2, rt2, a2, b2, c2)
                            end
                        end
        ...
        --getgenv().Drawing = drawing | this was actually causing the esp lag
    ...
    do -- UI Library
        ...
        --customFont.Data = bbot.theme.fontData
        local defaultProperties = {
            ...
            NumSides = 64,
            Visible = false,
            Font = bbot.useCustomFont and customFont or nil,
        }
        local themed = {
            accent = {},
            text = {},
        ...
        local function gradient(self, colorList, breaks) -- now this is pro
            ...
            for drawIdx = 1, breaks do
                ...
                square.Color = top
                        and top:Lerp(bottom, (drawIdx - 1) * offsetAmount)
                    or colorList[drawIdx]
                square.Filled = true
            ...
            return setmetatable({
                ...
                SetColor = function(self0, newcolor)
                    if type(newcolor) == 'table' then
                        ...
                        for drawIdx = 1, breaks do
                            squares[drawIdx].Color = newtop
                                    and newtop:Lerp(
                                        newbottom,
                                        (drawIdx - 1) * offsetAmount
                                    )
                                or newcolor[drawIdx]
                        end
                    else
                ...
                __newindex = function(self0, index, value)
                    for drawIdx = 1, breaks do
                        if index == 'Size' then
                            ...
                            squares[drawIdx].Position =
                                v2(pos.X, pos.Y + (drawIdx - 1) * boxSize)
                        elseif index == 'Position' then
                            pos = value
                            squares[drawIdx].Position = v2(
                                value.X,
                                value.Y + (drawIdx - 1) * (size / breaks)
                            )
        ...
        local function updateTheme(self) end
        ...
        local function addTextbox(self, text, default, callback, unsafe)
            local textbox = {}
            local visible = self.menu.open
                and self.tab.tabIndex == self.menu.tabIndex
                and self.index == 1
            ...
            textbox.height = 34
            textbox.buttonoutline = self.menu:draw(
                'Square',
                {
                    Position = container + v2(0, 15),
                    Size = v2(213, 18),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
                'outline'
            )
            textbox.button = modifyDrawing(
                self.menu:gradient(
                    { bbot.theme.lightbackground, bbot.theme.background },
                    6
                ),
                {
                    Position = textbox.buttonoutline.Position + v2(1, 1),
                    Size = v2(211, 16),
                    Color = bbot.theme.background,
                    Visible = visible,
                }
            )
            textbox.text = self.menu:draw(
                'Text',
                {
                    Position = textbox.buttonoutline.Position + v2(2, -16),
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = text,
                    Visible = visible,
                },
                'text'
            )
            textbox.valuetext = self.menu:draw(
                'Text',
                {
                    Position = textbox.button.Position + v2(6, 0),
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = textbox.value,
                    Visible = visible,
        ...
        local function addButton(self, text, callback, unsafe)
            ...
            button.height = 23
            button.buttonoutline = self.menu:draw(
                'Square',
                {
                    Position = container + v2(0, 3),
                    Size = v2(213, 18),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
            ...
            button.button = modifyDrawing(
                self.menu:gradient(
                    { bbot.theme.lightbackground, bbot.theme.background },
                    6
                ),
                {
                    Position = button.buttonoutline.Position + v2(1, 1),
                    Size = v2(211, 16),
                    Color = bbot.theme.background,
                    Visible = visible,
                }
            )
            button.text = self.menu:draw(
                'Text',
                {
                    Position = button.button.Position + v2(106, 0),
                    Center = true,
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = text,
        ...
        local function addDropdown(
            ...
            dropdown.height = 34
            dropdown.buttonoutline = self.menu:draw(
                'Square',
                {
                    Position = container + v2(0, 15),
                    Size = v2(213, 18),
            ...
            dropdown.button = modifyDrawing(
                self.menu:gradient(
                    { bbot.theme.lightbackground, bbot.theme.background },
                    6
                ),
                {
                    Position = dropdown.buttonoutline.Position + v2(1, 1),
                    Size = v2(211, 16),
                    Color = bbot.theme.background,
                    Visible = visible,
                }
            )
            dropdown.text = self.menu:draw(
                'Text',
                {
                    Position = dropdown.buttonoutline.Position + v2(2, -16),
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = text,
                    Visible = visible,
            ...
            dropdown.valuetext = self.menu:draw(
                'Text',
                {
                    Position = dropdown.button.Position + v2(6, 0),
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = dropdown.value,
                    Visible = visible,
            ...
            dropdown.droptext = self.menu:draw(
                'Text',
                {
                    Position = dropdown.button.Position + v2(200, 2),
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = '-',
                    Visible = visible,
        ...
        local function setSliderValue(self, value)
            ...
            self.highlight.Size =
                Vector2.new(math.clamp(ratio * 211, 0, 211), 9)
        ...
        local function addSlider(
            ...
            local ratio = (slider.default - slider.min)
                / (slider.max - slider.min)
            slider.buttonoutline = self.menu:draw(
                'Square',
                {
                    Position = container + v2(0, 16),
                    Size = v2(213, 11),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
            ...
            slider.button = modifyDrawing(
                self.menu:gradient(
                    { bbot.theme.lightbackground, bbot.theme.background },
                    3
                ),
                {
                    Position = slider.buttonoutline.Position + v2(1, 1),
                    Size = v2(211, 9),
                    Color = bbot.theme.background,
                    Visible = visible,
                }
            )
            slider.highlight = modifyDrawing(
                self.menu:gradient(
                    { bbot.theme.accent, darken(bbot.theme.accent, 0.25) },
                    3
                ),
                {
                    Position = slider.button.Position,
                    Size = v2(math.clamp(ratio * 211, 0, 211), 9),
                    Color = bbot.theme.accent,
                    Visible = visible,
                }
            )
            slider.text = self.menu:draw(
                'Text',
                {
                    Position = slider.buttonoutline.Position + v2(2, -15),
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = text,
                    Visible = visible,
            ...
            slider.valuetext = self.menu:draw(
                'Text',
                {
                    Position = slider.button.Position + v2(106, -3),
                    Size = 14,
                    Center = true,
                    Color = bbot.theme.text,
                    Text = tostring(default) .. slider.suffix,
                    Visible = visible,
                },
                'text'
            )
        ...
        local function addColorToToggle(self, name, default, callback)
            ...
            local visible = self.menu.open
                and self.tab.tabIndex == self.menu.tabIndex
                and self.sectionIndex == 1
            if name then
                self.section.flags[name] = color
            end
            default = default or bbot.theme.accent
            color.name = name or 'Color'
            color.callback = callback
            color.toggle = self
            color.value = default
            color.AddColorPicker = addColorToToggle
            color.buttonoutline = self.menu:draw(
                'Square',
                {
                    Position = self.buttonoutline.Position
                        + v2(187 - self.additions, 1),
                    Size = v2(26, 12),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
            ...
            color.buttonbackground = self.menu:draw(
                'Square',
                {
                    Position = color.buttonoutline.Position + v2(1, 1),
                    Size = v2(24, 10),
                    Color = darken(default, 0.4),
                    Visible = visible,
                },
                'accent'
            )
            color.button = self.menu:draw(
                'Square',
                {
                    Position = color.buttonoutline.Position + v2(3, 3),
                    Size = v2(20, 6),
                    Color = default,
                    Visible = visible,
                },
        ...
        local function addKeybindToToggle(self, default, name)
            if not self.keybind then
                ...
                keybind.buttonoutline = self.menu:draw(
                    'Square',
                    {
                        Position = self.buttonoutline.Position
                            + v2(171 - self.additions, 0),
                        Size = v2(42, 14),
                        Color = bbot.theme.outline,
                        Visible = visible,
                    },
                ...
                keybind.button = self.menu:draw(
                    'Square',
                    {
                        Position = keybind.buttonoutline.Position + v2(1, 1),
                        Size = v2(40, 12),
                        Color = bbot.theme.hidden,
                        Visible = visible,
                    },
                    'hidden'
                )
                keybind.text = self.menu:draw(
                    'Text',
                    {
                        Position = keybind.button.Position + v2(21, -2),
                        Size = 14,
                        Color = bbot.theme.text,
                        Text = default,
                        Center = true,
        ...
        local function setToggleValue(self, value)
            ...
            self.button:SetColor(
                value and { bbot.theme.accent, darken(bbot.theme.accent, 0.25) }
                    or { bbot.theme.lightbackground, bbot.theme.background }
            )
        ...
        local function addToggle(self, text, default, callback, unsafe)
            ...
            toggle.buttonoutline = self.menu:draw(
                'Square',
                {
                    Position = container + v2(0, 3),
                    Size = v2(11, 11),
            ...
            --toggle.button = self.menu:draw("Square", {Position = toggle.buttonoutline.Position + v2(1, 1), Size = v2(9, 9), Color = default and bbot.theme.accent or bbot.theme.background, Visible = visible}, "background")
            toggle.button = modifyDrawing(
                self.menu:gradient(
                    { bbot.theme.accent, darken(bbot.theme.accent, 0.25) },
                    3
                ),
                {
                    Position = toggle.buttonoutline.Position + v2(1, 1),
                    Size = v2(9, 9),
                    Color = default and bbot.theme.accent
                        or bbot.theme.background,
                    Visible = visible,
                }
            )
            toggle.text = self.menu:draw(
                'Text',
                {
                    Position = toggle.buttonoutline.Position + v2(16, -2),
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = text,
                    Visible = visible,
            ...
            toggle.button:SetColor(
                not default
                    and { bbot.theme.lightbackground, bbot.theme.background }
            )
            return toggle
        end
        ...
        local function addSection(self, text)
            ...
            section.buttonoutline = self.menu:draw(
                'Square',
                {
                    Position = lastButton.Position
                        + v2(lastButton.Size.X + 1, 0),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
                'outline'
            )
            section.button = self.menu:draw(
                'Square',
                {
                    Position = section.buttonoutline.Position + v2(0, 0),
                    Color = bbot.theme.hidden,
                    Visible = visible,
                },
                'hidden'
            )
            section.text = self.menu:draw(
                'Text',
                {
                    Size = 14,
                    Color = bbot.theme.hiddenText,
                    Center = true,
                    Text = text,
                    Visible = visible,
                },
            ...
            section.text.Position = section.button.Position
                + v2(5 + section.text.TextBounds.X * 0.5, 4)
        ...
        local function createSection(self, text, right, height)
            local section = {}
            local visible = self.menu.open
                and self.tabIndex == self.menu.tabIndex
            local side = right and 'right' or 'left'
            self.menu.sectionIndexes[text] = section
            height = height == 'half' and 257
                or height == 'whole' and 518
                or height == 'third' and 170
                or height
            section.outline = self.menu:draw(
                'Square',
                {
                    Size = v2(231, height),
                    Position = self.menu.sectionbg.Position
                        + v2(7 + (right and 235 or 0), 3 + self[side]),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
                'outline'
            )
            section.highlightoutline = self.menu:draw(
                'Square',
                {
                    Size = v2(229, 4),
                    Position = section.outline.Position + v2(1, 1),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
                'outline'
            )
            section.highlight = modifyDrawing(
                self.menu:gradient(
                    {
                        bbot.theme.accent:Lerp(Color3.new(1, 1, 1), 0.1),
                        bbot.theme.accent,
                        darken(bbot.theme.accent, 0.4),
                    },
                    3
                ),
                {
                    Size = v2(229, 3),
                    Position = section.highlightoutline.Position,
                    Color = bbot.theme.accent,
                    Visible = visible,
                }
            )
            section.buttons = self.menu:draw(
                'Square',
                {
                    Size = v2(229, 20),
                    Position = section.highlightoutline.Position + v2(0, 4),
                    Color = bbot.theme.hidden,
                    Visible = visible,
                },
                'hidden'
            ...
            section.buttonbackground = modifyDrawing(
                self.menu:gradient(
                    { bbot.theme.lightbackground, bbot.theme.background },
                    8
                ),
            ...
            section.buttonoutline.Size = v2(11 + section.text.TextBounds.X, 21)
            section.text.Position = section.buttons.Position
                + v2(5 + section.text.TextBounds.X * 0.5, 4)
            section.text.ZIndex = 3
            section.background = self.menu:draw(
                'Square',
                {
                    Size = v2(229, height - 27),
                    Position = section.outline.Position + v2(1, 26),
                    Color = bbot.theme.background,
                    Visible = visible,
                },
                'background'
            ...
            local function updateListText()
                ...
                for playerIndex = 1, 9 do
                    ...
                    if player then
                        ...
                        playerdrawings.team.Text = isteamed and player.Team.Name
                            or 'None'
                        playerdrawings.team.Color = isteamed
                                and player.TeamColor.Color
                            or bbot.theme.text
                        playerdrawings.status.Text = islocal and 'Local Player'
                            or status[player]
                            or 'None'
                        playerdrawings.status.Color = islocal
                                and Color3.new(0.407843, 0, 0.87451)
                            or bbot.theme.text
                    end
                end
            end
        ...
        local function createPlayerList(self, statuslist, callbacks)
            ...
            playerlist.outline = self.menu:draw(
                'Square',
                {
                    Size = v2(231 + 235, height),
                    Position = self.menu.sectionbg.Position + v2(7, 3),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
                'outline'
            )
            playerlist.highlightoutline = self.menu:draw(
                'Square',
                {
                    Size = v2(229 + 235, 4),
                    Position = playerlist.outline.Position + v2(1, 1),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
                'outline'
            )
            playerlist.highlight = modifyDrawing(
                self.menu:gradient(
                    {
                        bbot.theme.accent:Lerp(Color3.new(1, 1, 1), 0.1),
                        bbot.theme.accent,
                ...
                {
                    Size = v2(229 + 235, 3),
                    Position = playerlist.highlightoutline.Position,
                    Color = bbot.theme.accent,
                    Visible = visible,
                }
            )
            playerlist.buttons = self.menu:draw(
                'Square',
                {
                    Size = v2(229 + 235, 20),
                    Position = playerlist.highlightoutline.Position + v2(0, 4),
                    Color = bbot.theme.hidden,
                    Visible = visible,
                },
                'hidden'
            )
            playerlist.buttonoutline = self.menu:draw(
                'Square',
                {
                    Position = playerlist.highlightoutline.Position + v2(0, 4),
                    Color = bbot.theme.outline,
            ...
            playerlist.button = self.menu:draw(
                'Square',
                {
                    Position = playerlist.highlightoutline.Position + v2(0, 4),
                    Color = bbot.theme.background,
            ...
            playerlist.buttonbackground = modifyDrawing(
                self.menu:gradient(
                    { bbot.theme.lightbackground, bbot.theme.background },
                    8
                ),
            ...
            playerlist.text = self.menu:draw(
                'Text',
                {
                    Size = 14,
                    Color = bbot.theme.text,
            ...
            playerlist.buttonoutline.Size =
                v2(11 + playerlist.text.TextBounds.X, 21)
            playerlist.text.Position = playerlist.buttons.Position
                + v2(5 + playerlist.text.TextBounds.X * 0.5, 4)
            playerlist.text.ZIndex = 3
            playerlist.background = self.menu:draw(
                'Square',
                {
                    Size = v2(229 + 235, height - 27),
                    Position = playerlist.outline.Position + v2(1, 26),
                    Color = bbot.theme.background,
                    Visible = visible,
                },
                'background'
            )
            playerlist.playerBoxOutline = self.menu:draw(
                'Square',
                {
                    Size = v2(229 + 235 - 16, 210),
                    Position = playerlist.outline.Position + v2(9, 26 + 18),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
                'outline'
            )
            playerlist.playerBoxBackground = self.menu:draw(
                'Square',
                {
                    Size = v2(229 + 235 - 16 - 2, 208),
                    Position = playerlist.outline.Position + v2(10, 26 + 19),
                    Color = bbot.theme.background,
                    Visible = visible,
                },
                'background'
            )
            playerlist.playerPFPOutline = self.menu:draw(
                'Square',
                {
                    Size = v2(74, 74),
                    Position = playerlist.outline.Position
                        + v2(9, 26 + 18 + 8 + 210),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
            ...
            playerlist.playerPFP = self.menu:draw(
                'Image',
                {
                    Size = v2(72, 72),
                    Position = playerlist.outline.Position
            ...
            --playerlist.playerPFP = self.menu:draw("Square", {Size = v2(72, 72), Position = playerlist.outline.Position + v2(10, 26 + 18 + 8 + 211), Color = bbot.theme.outline, Visible = visible}, "outline")
            playerlist.statusbuttonoutline = self.menu:draw(
                'Square',
                {
                    Size = v2(149, 20),
                    Position = playerlist.outline.Position
            ...
            playerlist.statusbutton = modifyDrawing(
                self.menu:gradient(
                    { bbot.theme.lightbackground, bbot.theme.background },
                    6
                ),
            ...
            playerlist.votekickbuttonoutline = self.menu:draw(
                'Square',
                {
                    Size = v2(69, 20),
                    Position = playerlist.outline.Position
            ...
            playerlist.votekickbutton = modifyDrawing(
                self.menu:gradient(
                    { bbot.theme.lightbackground, bbot.theme.background },
                    6
                ),
            ...
            playerlist.spectatebuttonoutline = self.menu:draw(
                'Square',
                {
                    Size = v2(69, 20),
                    Position = playerlist.outline.Position
                        + v2(308 + 80, 26 + 18 + 210 + 53),
                    Color = bbot.theme.outline,
                    Visible = visible,
                },
                'outline'
            )
            playerlist.spectatebutton = modifyDrawing(
                self.menu:gradient(
                    { bbot.theme.lightbackground, bbot.theme.background },
                    6
                ),
                {
                    Size = v2(67, 18),
                    Position = playerlist.outline.Position
                        + v2(308 + 81, 26 + 18 + 210 + 54),
                    Visible = visible,
                }
            )

            playerlist.nametext = self.menu:draw(
                'Text',
                {
                    Position = playerlist.playerBoxOutline.Position
                        + v2(4, -17),
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = 'Name',
            ...
            playerlist.teamtext = self.menu:draw(
                'Text',
                {
                    Position = playerlist.playerBoxOutline.Position
                        + v2(4 + 148, -17),
            ...
            playerlist.statuslabeltext = self.menu:draw(
                'Text',
                {
                    Position = playerlist.playerBoxOutline.Position
                        + v2(4 + 298, -17),
            ...
            playerlist.playertext = self.menu:draw(
                'Text',
                {
                    Position = playerlist.playerPFP.Position + v2(78, -2),
                    Size = 14,
                    Color = bbot.theme.text,
            ...
            playerlist.playerstatustext = self.menu:draw(
                'Text',
                {
                    Position = playerlist.statusbuttonoutline.Position
                        + v2(-1, -17),
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = 'Player Status',
            ...
            playerlist.statustext = self.menu:draw(
                'Text',
                {
                    Position = playerlist.playerstatustext.Position + v2(6, 19),
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = 'None',
                    Visible = visible,
            ...
            playerlist.droptext = self.menu:draw(
                'Text',
                {
                    Position = playerlist.playerstatustext.Position
                        + v2(137, 20),
                    Size = 14,
                    Color = bbot.theme.text,
                    Text = '-',
            ...
            playerlist.votekicktext = self.menu:draw(
                'Text',
                {
                    Position = playerlist.votekickbutton.Position + v2(33, 1),
                    Size = 14,
                    Center = true,
                    Color = bbot.theme.text,
                    Text = 'Votekick',
            ...
            playerlist.spectatetext = self.menu:draw(
                'Text',
                {
                    Position = playerlist.spectatebutton.Position + v2(33, 1),
                    Size = 14,
                    Center = true,
                    Color = bbot.theme.text,
                    Text = 'Spectate',
            ...
            playerlist.carrot = self.menu:draw(
                'Text',
                {
                    Position = playerlist.playerBoxBackground.Position
                        + v2(437, -1),
                    Size = 14,
                    Outline = false,
                    Color = bbot.theme.accent,
            ...
            playerlist.tinyv = self.menu:draw(
                'Text',
                {
                    Position = playerlist.playerBoxBackground.Position
                        + v2(437, 195),
            ...
            for playerIndex = 1, 9 do
                ...
                if playerIndex ~= 9 then
                    drawinglist.sectionline = self.menu:draw(
                        'Square',
                        {
                            Size = v2(438, 1),
                            Position = playerlist.playerBoxBackground.Position
                ...
                drawinglist.teamline = self.menu:draw(
                    'Square',
                    {
                        Size = v2(1, playerIndex ~= 9 and 16 or 17),
                        Position = playerlist.playerBoxBackground.Position
                            + v2(148, 23 * (playerIndex - 1) + 4),
                        Color = bbot.theme.outline,
                        Visible = visible,
                    },
                    'outline'
                )
                drawinglist.statusline = self.menu:draw(
                    'Square',
                    {
                        Size = v2(1, playerIndex ~= 9 and 16 or 17),
                        Position = playerlist.playerBoxBackground.Position
                            + v2(298, 23 * (playerIndex - 1) + 4),
                        Color = bbot.theme.outline,
                        Visible = visible,
                    },
                    'outline'
                )
                drawinglist.name = self.menu:draw(
                    'Text',
                    {
                        Position = playerlist.playerBoxBackground.Position
                            + v2(4, 23 * (playerIndex - 1) + 5),
                        Size = 14,
                        Color = bbot.theme.text,
                        Text = '',
                        Visible = visible,
                ...
                drawinglist.team = self.menu:draw(
                    'Text',
                    {
                        Position = playerlist.playerBoxBackground.Position
                            + v2(152, 23 * (playerIndex - 1) + 5),
                        Size = 14,
                        Color = bbot.theme.text,
                        Text = '',
                        Visible = visible,
                ...
                drawinglist.status = self.menu:draw(
                    'Text',
                    {
                        Position = playerlist.playerBoxBackground.Position
                            + v2(302, 23 * (playerIndex - 1) + 5),
                        Size = 14,
                        Color = bbot.theme.text,
                        Text = '',
                        Visible = visible,
        ...
        local function createTab(self, text)
            ...
            tab.title = self:draw(
                ...
                {
                    Size = 15,
                    Position = tab.button.Position + v2(48, 11),
                    Color = bbot.theme[tab.tabIndex == self.tabIndex and 'text' or 'hiddenText'],
                    Text = text,
                    Center = true,
                    Visible = self.open,
                },
        ...
        local function createKeyList(self, includeKeyName)
            ...
            keys.outline = keys:draw(
                'Square',
                { Color = bbot.theme.outline, Visible = true },
                'outline'
            )
            keys.background = keys:draw(
                'Square',
                { Color = bbot.theme.background, Visible = true },
                'background'
            )
            keys.titlebackground = modifyDrawing(
                keys:gradient(
                    { bbot.theme.lightbackground, bbot.theme.background },
                    7
                ),
                { Color = bbot.theme.accent, Visible = true }
            )
            keys.highlightoutline = keys:draw(
                'Square',
                { Color = bbot.theme.outline, Visible = true },
                'outline'
            )
            keys.highlight = modifyDrawing(
                keys:gradient(
                    {
                        bbot.theme.accent:Lerp(Color3.new(1, 1, 1), 0.1),
                        bbot.theme.accent,
            ...
            local function updateKeybinds()
                ...
                for _, keyData in self.keybinds do
                    ...
                    local text = keybind.toggle.section.text.Text
                        .. ': '
                        .. keybind.toggle.text.Text
            ...
            local function updateList()
                ...
                keys.outline.Position = v2(
                    9,
                    workspace.CurrentCamera.ViewportSize.Y * 0.5
                        - height * 0.5
                        - 1
                )
                ...
                keys.titlebackground.Position = keys.background.Position
                    + v2(0, 5)
                ...
                for keyIndex = 1, keycount do
                    ...
                    text.Position = keys.title.Position
                        + v2(0, keyIndex * 16 + 1)
                end
            end
        ...
        function bbot:CreateMenu(title, visible, index)
            if visible == nil then
                visible = true
            end
            ...
            menu.outline = menu:draw(
                'Square',
                {
                    Size = bgSize + v2(2, 2),
                    Position = middle - bgSize * 0.5 - v2(1, 1),
                    Color = self.theme.outline,
                    Visible = visible,
                },
                'outline'
            )
            menu.background = menu:draw(
                'Square',
                {
                    Size = bgSize,
                    Position = middle - bgSize * 0.5,
                    Color = self.theme.background,
                    Visible = visible,
                },
                'background'
            )
            menu.outline2 = menu:draw(
                'Square',
                {
                    Size = v2(502, 4),
                    Position = menu.outline.Position,
                    Color = self.theme.outline,
                    Visible = visible,
                },
                'outline'
            )
            menu.highlightoutline = menu:draw(
                'Square',
                {
                    Size = v2(500, 4),
                    Position = menu.background.Position,
                    Color = self.theme.outline,
                    Visible = visible,
                },
                'outline'
            )
            menu.highlight = modifyDrawing(
                menu:gradient(
                    {
                        self.theme.accent:Lerp(Color3.new(1, 1, 1), 0.1),
                        self.theme.accent,
                        darken(self.theme.accent, 0.4),
                    },
                    3
                ),
                {
            ...
            menu.titlebackground = modifyDrawing(
                menu:gradient(
                    { self.theme.lightbackground, self.theme.background },
                    7
                ),
                {
                    Size = v2(500, 21),
                    Position = menu.background.Position + v2(0, 4),
                    Color = self.theme.accent,
                    Visible = visible,
                }
            )
            menu.title = menu:draw(
                'Text',
                {
                    Size = 16,
                    Position = menu.background.Position + v2(5, 5),
            ...
            menu.inline = menu:draw(
                'Square',
                {
                    Size = bgSize + v2(2 - 20, 2 - 35),
                    Position = menu.outline.Position + v2(10, 25),
                    Color = self.theme.outline,
                    Visible = visible,
                },
                'outline'
            ...
            menu.tab1 = menu:draw(
                ...
                {
                    Size = v2(95, 35),
                    Position = menu.inline.Position + v2(1, 3),
                    Color = self.theme.hidden,
                    Visible = visible,
                },
                'hidden'
            )
            menu.tab2 = menu:draw(
                ...
                {
                    Size = v2(95, 35),
                    Position = menu.tab1.Position + v2(96, 0),
                    Color = self.theme.hidden,
                    Visible = visible,
                },
                'hidden'
            ...
            menu.tab3 = menu:draw(
                ...
                {
                    Size = v2(96, 35),
                    Position = menu.tab2.Position + v2(96, 0),
                    Color = self.theme.hidden,
                    Visible = visible,
                },
                'hidden'
            ...
            menu.tab4 = menu:draw(
                ...
                {
                    Size = v2(95, 35),
                    Position = menu.tab3.Position + v2(97, 0),
                    Color = self.theme.hidden,
                    Visible = visible,
                },
                'hidden'
            ...
            menu.tab5 = menu:draw(
                ...
                {
                    Size = v2(95, 35),
                    Position = menu.tab4.Position + v2(96, 0),
                    Color = self.theme.hidden,
                    Visible = visible,
                },
                'hidden'
            )
            menu.tabbackground = modifyDrawing(
                menu:gradient(
                    { self.theme.lightbackground, self.theme.background },
                    14
                ),
            ...
            menu.inlightoutline = menu:draw(
                'Square',
                {
                    Size = v2(480, 4),
                    Position = menu.inline.Position + v2(1, 1),
                    Color = self.theme.outline,
                    Visible = visible,
                },
                'outline'
            ...
            menu.inlight = modifyDrawing(
                menu:gradient(
                    {
                        self.theme.accent:Lerp(Color3.new(1, 1, 1), 0.20),
                        self.theme.accent,
                        darken(self.theme.accent, 0.4),
                    },
                    3
                ...
                {
                    Size = v2(480, 3),
                    Position = menu.inlightoutline.Position,
                    Color = self.theme.accent,
                    Visible = visible,
                }
            )
            menu.sectionbg = menu:draw(
                'Square',
                {
                    Size = v2(480, 527),
                    Position = menu.inlight.Position + v2(0, 38),
                    Color = self.theme.background,
                    Visible = visible,
                },
                'background'
                ...
                if input.KeyCode ~= Enum.KeyCode.Unknown then
                    ...
                    if typing then
                        ...
                        then
                            task.spawn(function()
                                local screenGui =
                                    Instance.new('ScreenGui', game.CoreGui) -- skidded af + detected... Don't recommend using CoreGui but oh well
                                local textBox =
                                    Instance.new('TextBox', screenGui) -- detected
                            ...
                            local lower = not userInputService:IsKeyDown(
                                Enum.KeyCode.LeftShift
                            ) and not userInputService:IsKeyDown(
                    ...
                    for _, menuData in bbot.menus do
                        for _, keyData in menuData.keybinds do
                            if key == keyData[1] then
                                keyData[2].toggle:SetValue(
                                    not keyData[2].toggle.value
                                )

                                if
                                    menuData.keys and menuData.keys.updateList
                                then
                    ...
                    if
                        input.KeyCode == Enum.KeyCode[bbot.toggleKeybind]
                        and clockTime - lastToggle > fadeDuration * 2
                        and not dragging
                        and not sliding
                        and not dropping
                        and not picking
                    then
                        bbot.open = not bbot.open
                        lastToggle = clockTime

                        for _, menu in bbot.menus do
                            local trans = (bbot.open and 0 or 1) --menu.background.Transparency
                            local stepFactor = 1 / fadeSteps
                            local step = stepFactor * (bbot.open and 1 or -1)
                            ...
                            for _, drawing in
                                getTabDrawings(menu.tabs[menu.tabIndex])
                            do
                                insert(drawings, drawing)
                            end
            ...
            userInputService.InputChanged:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseWheel then
                    for _, list in playerlists do
                        if
                            list.playerBoxBackground.Visible
                            and checkDrawing(
                                userInputService:GetMouseLocation(),
                                list.playerBoxBackground
                            )
                        then
                            list.scrollcount = math.max(
                                math.min(
                                    list.scrollcount
                                        + ((input.Position.Z > 0) and 1 or -1),
                                    0
                                ),
                                -#list.playerdata + 9
                            )
        ...
        table.insert(
            ...
            runService.RenderStepped:Connect(function(delta)
                if tick() - lastMenuRefresh < 1 / 30 then
                    return
                end
                lastMenuRefresh = tick()
                local down = userInputService:IsMouseButtonPressed(
                ...
                if bbot.open then
                    if sliding then
                        if down then
                            local relpos = userInputService:GetMouseLocation().X
                                - sliding.button.Position.X
                            local fraction = math.clamp(relpos, 0, 211) / 211
                            local range = sliding.max - sliding.min
                            local factor = math.floor(
                                range * fraction / sliding.step + 0.5
                            )
                            sliding:SetValue(
                                sliding.min + factor * sliding.step
                            )
                            return
                        else
                            sliding = false
                    ...
                    if statusdropping then
                        if clicked then
                            ...
                            if selectedstatus then
                                ...
                                statusdropping.list.playerstatus[player] =
                                    selectedstatus
                                statusdropping.list.status(
                                    player,
                                    selectedstatus
                                )
                    ...
                    if picking then
                        ...
                        if clicked then
                            if checkDrawing(mouse, picker.outline) then
                                ...
                                elseif
                                    checkDrawing(mouse, picker.hsvOutline)
                                then
                                    picker.clicked =
                                        { 'hsv', clock, Vector2.zero }
                                elseif checkDrawing(mouse, picker.hue) then
                                    picker.clicked =
                                        { 'hue', clock, Vector2.zero }
                                elseif checkDrawing(mouse, picker.valuebar) then
                                    picker.clicked =
                                        { 'val', clock, Vector2.zero }
                                elseif
                                    checkBounds(
                                        mouse
                                            - (
                        ...
                        elseif down then
                            ...
                            if picker.dragging then
                                local oldPos, oldTime =
                                    table.unpack(picker.dragging)
                            ...
                            if picker.clicked then
                                local clicktype, oldTime, oldMouse =
                                    table.unpack(picker.clicked)

                                if
                                    clock > oldTime + pickerUpdateRate
                                    and (mouse - oldMouse).Magnitude > 0
                                then
                                    if clicktype == 'hsv' then
                                        local bgpos = picker.hsvOutline.Position
                                            + v2(1, 1)
                                        local rel = mouse - bgpos
                                        local x, y =
                                            math.clamp(rel.X, 0, 166),
                                            math.clamp(rel.Y, 0, 166)
                                        local sat = x / 166
                                        local val = 1 - (y / 166)
                                        picker.newColor.Color = Color3.fromHSV(
                                            picker.current[1],
                                            sat,
                                            val
                                        )
                                        ...
                                        picker.hsvButtonOutline.Position = bgpos
                                            + v2(x - 3, y - 3)
                                        picker.hsvButton.Position = bgpos
                                            + v2(x - 2, y - 2)
                                        picker.valueButtonOutline.Position = v2(
                                            picker.valuebar.Position.X
                                                + 166
                                                - y
                                                - 3,
                                            picker.valueButtonOutline.Position.Y
                                        )
                                        picker.valueButton.Position = picker.valueButtonOutline.Position
                                            + v2(1, 1)
                                    elseif clicktype == 'val' then
                                        local x = math.clamp(
                                            mouse.X - picker.valuebar.Position.X,
                                            0,
                                            166
                                        )
                                        local val = x / 166
                                        ...
                                        picker.current[3] = val
                                        picker.valueButtonOutline.Position = v2(
                                            picker.valuebar.Position.X + x - 3,
                                            picker.valueButtonOutline.Position.Y
                                        )
                                        picker.valueButton.Position = picker.valueButtonOutline.Position
                                            + v2(1, 1)
                                        picker.hsvButtonOutline.Position = v2(
                                            picker.hsvButtonOutline.Position.X,
                                            picker.hsvOutline.Position.Y
                                                + 166
                                                - x
                                                - 2
                                        )
                                        picker.hsvButton.Position = picker.hsvButtonOutline.Position
                                            + v2(1, 1)
                                    elseif clicktype == 'hue' then
                                        local y = math.clamp(
                                            mouse.Y - picker.hue.Position.Y,
                                            0,
                                            166
                                        )
                                        local hue = (166 - y) / 166
                                        ...
                                        picker.hueButtonOutline.Position = v2(
                                            picker.hueButtonOutline.Position.X,
                                            picker.hue.Position.Y + y - 3
                                        )
                                        picker.hueButton.Position = picker.hueButtonOutline.Position
                                            + v2(1, 1)
                                        picker.clicked[2] = clock
                                        picker.hueSquare.Color =
                    ...
                    for _, menuData in bbot.menus do
                        if menuData.open then
                            ...
                            local onInside = onMenu
                                and checkDrawing(mouse, menuData.inline)
                                ...
                                if mouse.Y < sectionbg.Y then
                                    if clicked then
                                        local newIndex = math.clamp(
                                            math.ceil(
                                                (mouse.X - sectionbg.X) / 96
                                            ),
                                            1,
                                            5
                                        )
                                        local oldTab =
                                            menuData.tabs[menuData.tabIndex]
                                        local newTab = menuData.tabs[newIndex]
                                        oldTab.title.Color =
                                            bbot.theme.hiddenText
                                        newTab.title.Color = bbot.theme.text
                                        local oldButton =
                                            menuData['tab' .. tostring(
                                                menuData.tabIndex
                                            )]
                                        local newButton =
                                            menuData['tab' .. tostring(newIndex)]
                                        oldButton.Size = oldButton.Size
                                            - v2(0, 1)
                                        newButton.Size = newButton.Size
                                            + v2(0, 1)
                                        oldButton.Color = bbot.theme.hidden
                                        newButton.Color = bbot.theme.background
                                        menuData.tabbackground.Position =
                                            newButton.Position
                                        menuData.tabbackground.Size =
                                            newButton.Size
                                    ...
                                    for _, sectionData in
                                        menuData.tabs[menuData.tabIndex].mainSections
                                    do
                                        if sectionData.type == 'playerlist' then
                                            if
                                                clicked
                                            ...
                                            then
                                                if
                                                    checkDrawing(
                                                        mouse,
                                                        sectionData.playerBoxBackground
                                                    )
                                                then
                                                    local index = math.min(
                                                        math.ceil(
                                                            (
                                                                mouse.Y
                                                                - sectionData.playerBoxBackground.Position.Y
                                                            )
                                                                / 23
                                                        ),
                                                        9
                                                    ...
                                                    local player =
                                                        sectionData.playerdata[index - sectionData.scrollcount]
                                                    ...
                                                    then
                                                        sectionData.selected =
                                                            player
                                                        sectionData.playerPFP.Data = players:GetUserThumbnailAsync(
                                                            player.UserId,
                                                            Enum.ThumbnailType.HeadShot,
                                                            Enum.ThumbnailSize.Size420x420
                                                        ) or blankData
                                                        sectionData.playertext.Text =
                                                            player.Name
                                                    end
                                                end
                                                ...
                                                if sectionData.selected then
                                                    if
                                                        checkDrawing(
                                                            mouse,
                                                            sectionData.votekickbuttonoutline
                                                        )
                                                    then
                                                        sectionData.votekick(
                                                            sectionData.selected
                                                        )
                                                    end
                                                    ...
                                                    if
                                                        checkDrawing(
                                                            mouse,
                                                            sectionData.spectatebuttonoutline
                                                        )
                                                    then
                                                        sectionData.votekick(
                                                            sectionData.selected
                                                    ...
                                                    local statusButton =
                                                        sectionData.statusbuttonoutline
                                                    if
                                                        checkDrawing(
                                                            mouse,
                                                            statusButton
                                                    ...
                                                    then
                                                        ...
                                                        for statusIndex = 1, #sectionData.statuslist + 1 do
                                                            local statusName = statusIndex
                                                                        == 1
                                                                    and 'None'
                                                                or sectionData.statuslist[statusIndex - 1]
                                                            local buttonData =
                                                                {}
                                                            buttonData.status =
                                                                statusName
                                                            buttonData.outline =
                                                                statusdropping:draw(
                                                                    'Square',
                                                                    {
                                                                        ...
                                                                        Position = statusButton.Position
                                                                            + v2(
                                                                                0,
                                                                                19
                                                                                    * statusIndex
                                                            ...
                                                            buttonData.button =
                                                                statusdropping:draw(
                                                                    'Square',
                                                                    {
                                                                        ...
                                                                        Position = buttonData.outline.Position
                                                                            + v2(
                                                                                1,
                                                                                1
                                                                            ),
                                                            ...
                                                            statusdropping.buttons[statusIndex] =
                                                                buttonData
                                                        end
                                                    end
                                                end
                                        ...
                                        then
                                            if
                                                mouse.Y
                                                < sectionData.background.Position.Y
                                            then
                                                if clicked then
                                                    for sectionIndex, section in
                                                    ...
                                                    do
                                                        ...
                                                        then
                                                            local oldSec =
                                                                sectionData.sections[sectionData.sectionIndex]
                                                            oldSec.text.Color =
                                                                bbot.theme.hiddenText
                                                            section.text.Color =
                                                                bbot.theme.text
                                                            oldSec.button.Size = oldSec.button.Size
                                                                - v2(0, 1)
                                                            section.button.Size = section.button.Size
                                                                + v2(0, 1)
                                                            oldSec.button.Color =
                                                                bbot.theme.hidden
                                                            section.button.Color =
                                                                bbot.theme.background
                                                            sectionData.buttonbackground.Position =
                                                                section.button.Position
                                                            sectionData.buttonbackground.Size =
                                                                section.button.Size
                                                ...
                                                if clicked then
                                                    ...
                                                    local relPos = mouse
                                                        - (
                                                            sectionData.background.Position
                                                            + v2(8, 4)
                                                        )
                                                    local height = 0
                                                                    ...
                                                                    then
                                                                        ...
                                                                        picker.outline =
                                                                            picker:draw(
                                                                                'Square',
                                                                                {
                                                                                    Position = color.toggle.buttonoutline.Position
                                                                                        + v2(
                                                                                            -57,
                                                                                            -8
                                                                                        ),
                                                                        ...
                                                                        picker.background =
                                                                            picker:draw(
                                                                                'Square',
                                                                                {
                                                                                    Position = picker.outline.Position
                                                                                        + v2(
                                                                                            1,
                                                                                            1
                                                                                        ),
                                                                        ...
                                                                        picker.highlight =
                                                                            modifyDrawing(
                                                                                ...
                                                                                {
                                                                                    Position = picker.highlightbackground.Position,
                                                                                    Size = v2(
                                                                                        273,
                                                                                        3
                                                                                    ),
                                                                        ...
                                                                        picker.titlebackground =
                                                                            modifyDrawing(
                                                                                picker:gradient(
                                                                                    {
                                                                                        bbot.theme.lightbackground,
                                                                                        bbot.theme.background,
                                                                                    },
                                                                                    6
                                                                                ),
                                                                                {
                                                                                    ...
                                                                                    Position = picker.background.Position
                                                                                        + v2(
                                                                                            0,
                                                                                            4
                                                                                        ),
                                                                        ...
                                                                        picker.hueOutline =
                                                                            picker:draw(
                                                                                'Square',
                                                                                {
                                                                                    Position = picker.hsvOutline.Position
                                                                                        + v2(
                                                                                            7
                                                                                                + picker.hsvOutline.Size.X,
                                                                                            0
                                                                                        ),
                                                                                    Size = v2(
                                                                                        12,
                                                                        ...
                                                                        picker.newtext =
                                                                            picker:draw(
                                                                                'Text',
                                                                                {
                                                                                    Position = picker.hueOutline.Position
                                                                                        + v2(
                                                                                            19,
                                                                                            -2
                                                                                        ),
                                                                        ...
                                                                        picker.newOutline =
                                                                            picker:draw(
                                                                                'Square',
                                                                                {
                                                                                    Position = picker.newtext.Position
                                                                                        + v2(
                                                                                            0,
                                                                                            17
                                                                                        ),
                                                                        ...
                                                                        picker.oldOutline =
                                                                            picker:draw(
                                                                                'Square',
                                                                                {
                                                                                    Position = picker.oldtext.Position
                                                                                        + v2(
                                                                                            0,
                                                                                            17
                                                                                        ),
                                                                        ...
                                                                        local startPos = picker.hsvOutline.Position
                                                                            + v2(
                                                                                1,
                                                                                1
                                                                            )
                                                                        ...
                                                                        for i = 1, 2 do
                                                                            ...
                                                                            local uiGradient =
                                                                                Instance.new(
                                                                                    'UIGradient',
                                                                                    picker[i == 1 and 'satSquare' or 'valSquare']._data.drawings.box
                                                                                )
                                                                        ...
                                                                        picker.hsvButton =
                                                                            picker:draw(
                                                                                'Square',
                                                                                {
                                                                                    Position = picker.hsvButtonOutline.Position
                                                                                        + v2(
                                                                                            1,
                                                                                            1
                                                                                        ),
                                                                        ...
                                                                        picker.hueButtonOutline =
                                                                            picker:draw(
                                                                                'Square',
                                                                                {
                                                                                    Position = picker.hue.Position
                                                                                        + v2(
                                                                                            -3,
                                                                                            (
                                                                                                1
                                                                        ...
                                                                        picker.hueButton =
                                                                            picker:draw(
                                                                                'Square',
                                                                                {
                                                                                    Position = picker.hueButtonOutline.Position
                                                                                        + v2(
                                                                                            1,
                                                                                            1
                                                                                        ),
                                                                        ...
                                                                        picker.valueButtonOutline =
                                                                            picker:draw(
                                                                                'Square',
                                                                                {
                                                                                    Position = picker.valuebar.Position
                                                                                        + v2(
                                                                                            v
                                                                                                    * 166
                                                                                                - 3,
                                                                        ...
                                                                        picker.valueButton =
                                                                            picker:draw(
                                                                                'Square',
                                                                                {
                                                                                    Position = picker.valueButtonOutline.Position
                                                                                        + v2(
                                                                                            1,
                                                                                            1
                                                                                        ),
                                                                ...
                                                                for optionIndex = 1, #element.options do
                                                                    ...
                                                                    newDrawings.outline =
                                                                        menuData:draw(
                                                                            'Square',
                                                                            {
                                                                                Position = element.buttonoutline.Position
                                                                                    + v2(
                                                                                        0,
                                                                                        optionIndex
                                                                                            * 17
                                                                    ...
                                                                    newDrawings.button =
                                                                        menuData:draw(
                                                                            'Square',
                                                                            {
                                                                                Position = newDrawings.outline.Position
                                                                                    + v2(
                                                                                        1,
                                                                                        1
                                                                                    ),
                                                                    ...
                                                                    newDrawings.valuetext =
                                                                        menuData:draw(
                                                                            'Text',
                                                                            {
                                                                                Position = newDrawings.button.Position
                                                                                    + v2(
                                                                                        6,
                                                                                        0
                                                                                    ),
                                                                    ...
                                                                    element.optionDrawings[optionIndex] =
                                                                        newDrawings
                                                                end
                                                            elseif
                                                                element.type
    ...
    do -- Chams Library
        ...
        function cham.new(
            ...
            if model then
                ...
                table.insert(
                    connectionList,
                    model.DescendantAdded:Connect(classify)
                )
        ...
        table.insert(
            ...
            game:GetService('RunService').RenderStepped:Connect(function()
                if tick() - lastChamCheck < 1 / 60 then
                    return
                end
                lastChamCheck = tick()
...
LPH_JIT_MAX(function() -- Main Cheat
    ...
    for i, v in getgc(true) do
        if
            type(v) == 'table'
            and rawget(v, 'ScreenCull')
            and rawget(v, 'NetworkClient')
        then
            moduleCache = v
            break
        end
    ...
    --some say this script has too many index calls
    ...
    --local astar = loadstring(game:HttpGet("https://raw.githubusercontent.com/jensonhirst/Sirius/request/library/Pathfinding"))() 
    -- it flies so it despawns now. ill make pathfinding that stays on the ground
    --astar.maxtime = 0.33
    --astar.interval = 12  --  8 to 16 is good
    --astar.ignorelist = {workspace.Players, camera, ignore, hitboxObjects, backtrackObjects}

    local pathfinding = loadstring(
        game:HttpGet(
            'https://raw.githubusercontent.com/1337kat/P-h-a-n-t-o-m-F-o-r-c-e-s-/main/bbot/pathfinding.lua'
        )
    )() -- Skidded like a pro
        ...
        replicationInterface.operateOnAllEntries(function(player, entry)
            ...
            if character and player.Team ~= localplayer.Team then
                ...
                then
                    ...
                    if
                        screenPosition.Z > 0
                        and screenDistance < distance
                        and (not deadfov or screenDirection >= deadfov)
                    then
    ...
    local function getClosestPlayers(
        ...
        replicationInterface.operateOnAllEntries(function(player, entry)
            ...
            if
                entry._receivedPosition
                and entry._velspring.t
                and character
                ...
                and (
                    not ignoreCheck
                    or (
                        not killedPlayers[player] and not ignoredPlayers[player]
                    )
                )
            then
                if
                    (not useWhitelist or playerStatus[player] ~= 'Friendly')
                    and (not onlyTargets or playerStatus[player] == 'Target')
                then
    ...
    local function solve(v44, v45, v46, v47, v48) -- i did not write this
        ...
        elseif v44 > -1.0E-10 and v44 < 1.0E-10 then
            ...
            if v48 then
                ...
                local v51 = (
                    v47 + v49 * (2 * v46 + v49 * (3 * v45 + 4 * v44 * v49))
                ) / v44
                local v52 = (
                    v48
                    + v49 * (v47 + v49 * (v46 + v49 * (v45 + v44 * v49)))
                ) / v44
                if v51 > -1.0E-10 and v51 < 1.0E-10 then
                    ...
                    local v57, _, v59 =
                        solve(1, 2 * v50, v50 * v50 - 4 * v52, -v51 * v51)
            ...
            elseif v47 then
                ...
                local v68 = -(v47 + v66 * (v46 + v66 * (v45 + v44 * v66)))
                    / (2 * v44)
                ...
                if v69 > 0 then
                    local v71 = v68 + v70
                    local v72 = v68 - v70
                    v71 = v71 < 0 and -(-v71) ^ 0.3333333333333333
                        or v71 ^ 0.3333333333333333
                    local v73 = v72 < 0 and -(-v72) ^ 0.3333333333333333
                        or v72 ^ 0.3333333333333333
                    return v66 + v71 + v73
                else
    ...
    local function toanglesyx(v)
        ...
        return math.asin(y / (x * x + y * y + z * z) ^ 0.5),
            math.atan2(-x, -z),
            0
    end
    ...
    local function simulateBullet(origin, velocity, penetration)
        ...
        while newTime < 1 do
            ...
            local motion = (frameTime * newVelocity)
                + (((frameTime * frameTime) / 2) * frameAcceleration)
            local hit, enter = raycast(newOrigin, motion, ignoreList)

            if
                hit
                and hit.CanCollide
                and hit.Transparency ~= 1
                and hit.Name ~= 'Window'
            then
                ...
                local _, exit =
                    raycast(enter + maxExtent, -maxExtent, { hit }, true)
                ...
                if canShoot then
                    ...
                    local timePassed = (
                        motion:Dot(enter - newOrigin) / motion:Dot(motion)
                    ) * frameTime
    ...
    local function getPositionOffsets(origin, target, offset)
        if offset then
            local cframe = CFrame.new(origin, target)
                * CFrame.Angles(0, 0, math.rad(math.random(1, 90)))
            local offsets = {}

            for vertexIndex = 1, #scanVerticies do
                table.insert(
                    offsets,
                    cframe * (scanVerticies[vertexIndex] * offset)
                )
            end
    ...
    --local bulletIgnoreList = debug.getupvalue(bulletcheck, 4)--cant use this method anymore cuz im getting rid of getupvalue
    --table.insert(bulletIgnoreList, hitboxObjects) -- only adding this fix cuz sirmeme ran into this bug on stream
    --table.insert(bulletIgnoreList, backtrackObjects) -- robloxscripts.com
        ...
        if getfenv(ignoreFunc).script == getfenv(bulletcheck).script then
            ignoreFunc = function(part)
                ...
                elseif part.Transparency == 1 then
                    return true
                elseif
                    part:IsDescendantOf(hitboxObjects)
                    or part:IsDescendantOf(backtrackObjects)
                then
                    return true
                else
                    return
    ...
    local function scanPositions(origin, target, accel, speed, penetration)
        local origins = getPositionOffsets(
            origin,
            target,
            bbot:GetValue('Rage Bot', 'Fire Position Scanning')
                and bbot:GetValue('Rage Bot', 'Fire Position Offset')
        )
        local targets = getPositionOffsets(
            target,
            origin,
            bbot:GetValue('Rage Bot', 'Hit Position Scanning')
                and bbot:GetValue('Rage Bot', 'Hit Position Offset')
        )
        ...
        for originIndex = 1, #origins do
            ...
            for targetIndex = 1, #targets do
                ...
                local velocity, hitTime =
                    trajectory(newOrigin, accel, newTarget, speed)
    ...
    local function getBarrelLocation()
        ...
        return weapon
            and not weapon._aiming
            and weapon._barrelPart
            and camera:WorldToViewportPoint(
                weapon._barrelPart.CFrame * Vector3.new(0, 0, -100)
            )
    end
    ...
    local function applyAAAngles(angles)
        ...
        if bbot:GetValue('Anti Aim', 'Pitch') then
            local addition = rad(bbot:GetValue('Anti Aim', 'Pitch Amount'))
                - quarter

            if
                string.find(
                    string.lower(bbot:GetValue('Anti Aim', 'Pitch Mode')),
                    'abs'
                )
            then
                x = addition
        ...
        if bbot:GetValue('Anti Aim', 'Yaw') then
            local addition = rad(bbot:GetValue('Anti Aim', 'Yaw Amount'))

            if
                string.find(
                    string.lower(bbot:GetValue('Anti Aim', 'Yaw Mode')),
                    'rel'
                )
            then
                y += addition
        ...
        if bbot:GetValue('Anti Aim', 'Spin Bot') then
            y += (os.clock() - startTime) * math.rad(
                bbot:GetValue('Anti Aim', 'Spin Speed')
            ) * ((bbot:GetValue('Anti Aim', 'Spin Direction') == 'Left' and 1) or -1)
        end
    ...
    function network:send(name, ...)
        if
            bbot:GetValue('Third Person', 'Enabled')
            and bbot:GetValue('Third Person', 'Show Character')
        then
            if name == 'spawn' then
                if not started then
                    started = true
                ...
                elseif name == 'stance' then
                    ...
                    if
                        (
                            not bbot:GetValue(
                                'Anti Aim',
                                'Enabled (May Cause Despawning)'
                            )
                            or not bbot:GetValue('Anti Aim', 'Force Stance')
                            or not bbot:GetValue(
                                'Third Person',
                                'Apply Anti Aim To Character'
                            )
                        ) and currentObj
                    then
        ...
        elseif name == 'repupdate' then
            ...
            if teleporting then -- pf devs be like "noooooo"
                if not teleportData.time then
                    ...
                    send(
                        self,
                        name,
                        teleportData.path[index],
                        angles,
                        angles2,
                        time
                            + newSpawnCache.latency
                            + newSpawnCache.currentAddition
                    )
            ...
            if newSpawnCache.noclipping then -- this is fried and i broke it somehow. Noclip doesn't work anymore T_T
                if clockTime > newSpawnCache.noclipstart then
                    local rootPart =
                        charInterface.getCharacterObject():getRealRootPart()
                    local partList =
                        workspace:GetPartsInPart(rootPart, OverlapParams.new())
                    local touching = {}
                    ...
                    if #touching == 0 then
                        if
                            initTeleport(newSpawnCache.lastUpdate, position)
                            ~= false
                        then
                ...
                return
            elseif
                bbot:GetValue('Movement', 'Noclip')
                and newSpawnCache.lastUpdate
            then
                local hit = raycast(
                    newSpawnCache.lastUpdate,
            ...
            if newSpawnCache.updateDebt > 0 then -- idk what this does but screw it
                newSpawnCache.updateDebt -= 1
                return
            end

            if bbot:GetValue('Anti Aim', 'Fake Lag') then
                if
                    not fakelag.lastRefreshPosition
                    or not fakelag.lastRefreshTime
                then
                ...
                if
                    (
                        (position - fakelag.lastRefreshPosition).Magnitude
                        > bbot:GetValue('Anti Aim', 'Refresh Distance')
                    )
                    or tick() - fakelag.lastRefreshTime
                        > bbot:GetValue('Anti Aim', 'Refresh Rate')
                then
                    fakelag.lastRefreshPosition = position
                    fakelag.lastRefreshTime = tick()

                    if bbot:GetValue('Anti Aim', 'Randomize Position') then
                        local xaxis, yaxis, zaxis =
                            bbot:GetValue('Anti Aim', 'X-Axis Factor'),
                            0,
                            bbot:GetValue('Anti Aim', 'Z-Axis Factor')
                        local xoff, yoff, zoff =
                            math.random(-xaxis, xaxis),
                            math.random(-yaxis, yaxis),
                            math.random(-zaxis, zaxis)
            ...
            if bbot:GetValue('Anti Aim', 'Enabled (May Cause Despawning)') then
                angles = applyAAAngles(angles)
                angles2 = angles * 0.99
            end

            --if bbot:GetValue("Rage Bot", "Enabled") and bbot:GetValue("Rage Bot", "Firerate (May Cause Kicking)") then
            ...
            --local fly = false --bbot:GetValue("Movement", "Fly") or (bbot:GetValue("Rage Bot", "Enabled") and bbot:GetValue("Rage Bot", "Firerate (May Cause Kicking)"))
            ...
            if
                bbot:GetValue('Movement', 'Walk Speed')
                and newSpawnCache.lastUpdate
            then -- no patch pls :(
                send(
                    self,
                    ...
                    angles,
                    angles2,
                    time + newSpawnCache.latency + newSpawnCache.currentAddition
                )
                newSpawnCache.updateDebt += 1
            end
        ...
        elseif name == 'newbullets' then
            ...
            if bbot:GetValue('Rage Bot', 'Enabled') then
                return
            end

            if
                bbot:GetValue('Silent Aim', 'Enabled')
                and (bbot:GetValue('Silent Aim', 'Hit Chance') >= chanceOne)
            then
                local target, entry, part = getClosest(
                    silentaimfov.Position,
                    bbot:GetValue('Silent Aim', 'Use FOV')
                        and silentaimfov.Radius,
                    bbot:GetValue('Silent Aim', 'Use Dead FOV')
                        and silentaimdeadfov.Radius,
                    bbot:GetValue('Silent Aim', 'Visible Check'),
                    (
                        bbot:GetValue('Silent Aim', 'Head Shot Chance')
                        >= chanceTwo
                    )
                            and 'Head'
                        or 'Torso'
                ...
                if target then
                    ...
                    local velocity = complexTrajectory(
                        ...
                        target,
                        weaponInterface
                            .getActiveWeaponController()
                            :getActiveWeapon()._weaponData.bulletspeed,
                        (
                            movementCache.position[player][15]
                            - movementCache.position[player][1]
                        )
                            / (movementCache.time[15] - movementCache.time[1])
                    ).Unit
        ...
        elseif
            name == 'falldamage'
            and bbot:GetValue('Movement', 'No Fall Damage')
        then
            return
        elseif name == 'stance' then
        ...
        elseif timeUpdates[name] then
            ...
            if name == 'equip' then
                ...
                if
                    bbot:GetValue('Knife Bot', 'Kill All (May Despawn)')
                    and not bbot:GetValue(
                        'Knife Bot',
                        'Only When Holding Knife'
                    )
                then
                    args[1] = 3
                end
        ...
        elseif name == 'ping' then
            ...
            --if bbot:GetValue("Rage Bot", "Enabled") and bbot:GetValue("Rage Bot", "Firerate (May Cause Kicking)") then -- idk if this needs to be here i think it helps a little
        ...
        if bbot:GetValue('Anti Aim', 'Enabled (May Cause Despawning)') then
            if
                name == 'stance' and bbot:GetValue('Anti Aim', 'Force Stance')
            then
                local stance = ...
                stance = string.lower(bbot:GetValue('Anti Aim', 'Set Stance'))
    ...
    function weaponObject:preparePickUpFirearm(
        slot,
        name,
        attachments,
        attData,
    ...
    function weaponObject:preparePickUpMelee(
        name,
        camoData,
        newId,
        wasClient,
    ...
    screenCull.step = LPH_NO_VIRTUALIZE(function(...)
        ...
        if bbot:GetValue('Third Person', 'Enabled') then
            local controller = weaponInterface.getActiveWeaponController()
            ...
            if
                ...
                and (
                    bbot:GetValue(
                        'Third Person',
                        'Show Character While Aiming'
                    )
                    or not controller:getActiveWeapon()._aiming
                )
            then
                local cameraOffset = Vector3.new(
                    bbot:GetValue('Third Person', 'Camera Offset X'),
                    bbot:GetValue('Third Person', 'Camera Offset Y'),
                    bbot:GetValue('Third Person', 'Camera Offset Z')
                )
                local didHit = false
                ...
                then
                    ...
                    if hit then
                        camera.CFrame *= CFrame.new(
                            cameraOffset
                                * ((position - oldPosition).Magnitude / cameraOffset.Magnitude)
                                * 0.99
                        )
                        didHit = true
                    end
    ...
    function bulletObject.new(bulletData)
        if bulletData.onplayerhit then
            if unlockAll then
                ...
                local name = fakeWeapons[playerDataUtils.getClassData(
                    playerClient.getPlayerData()
                ).curclass][controller:getActiveWeaponIndex()]

                if displayname == name then
                    local serverSpeed =
                        contentDatabase.getWeaponData(name).bulletspeed
                ...
                if target then
                    ...
                    if target and movementCache.position[player] then
                        ...
                        local velocity = complexTrajectory(
                            ...
                            movementCache.position[player][15]
                                    and (movementCache.position[player][15] - movementCache.position[player][1]) / (movementCache.time[15] - movementCache.time[1])
                                or Vector3.zero
                        )
            ...
            if
                bbot:GetValue('World Visuals', 'Bullet Tracers')
                or bbot:GetValue('World Visuals', 'Impact Points')
            then
                local frames, hits = simulateBullet(
                    bulletData.position,
                    bulletData.velocity,
                    bulletData.penetrationdepth
                )

                if bbot:GetValue('World Visuals', 'Bullet Tracers') then
                    local endColor =
                        bbot:GetValue('World Visuals', 'Color One')
                    local startColor =
                        bbot:GetValue('World Visuals', 'Color Two')
                    local diameter =
                        bbot:GetValue('World Visuals', 'Tracers Size')
                    ...
                    for frame = 1, frameCount do -- god damn perfect bullet tracers
                        ...
                        tracer.Material = Enum.Material[bbot:GetValue(
                            'World Visuals',
                            'Tracers Material'
                        )]
                        tracer.Transparency = bbot:GetValue(
                            'World Visuals',
                            'Tracers Transparency'
                        ) * 0.01
                        tracer.Anchored = true
                        tracer.CanCollide = false
                        tracer.Color = startColor:lerp(
                            endColor,
                            (frame - 1) / math.max(frameCount - 1, 1)
                        )
                        ...
                        tracer.CFrame = (
                            ...
                            * CFrame.new(Vector3.new(distance * 0.5, 0, 0))
                        tracer.Parent = ignore

                        task.delay(
                            bbot:GetValue('World Visuals', 'Duration'),
                            function()
                                local step = (1 - tracer.Transparency) / 10

                                for i = 1, 10 do
                                    tracer.Transparency = tracer.Transparency
                                        + step
                                    task.wait(0.05)
                                end
                ...
                if bbot:GetValue('World Visuals', 'Impact Points') then
                    for wall = 1, #hits do
                        local point = Instance.new('Part')
                        point.Material = Enum.Material[bbot:GetValue(
                            'World Visuals',
                            'Points Material'
                        )]
                        point.Transparency = bbot:GetValue(
                            'World Visuals',
                            'Points Transparency'
                        ) * 0.01
                        point.Anchored = true
                        ...
                        point.Color =
                            bbot:GetValue('World Visuals', 'Points Color')
            ...
            if
                bbot:GetValue('Backtracking', 'Enabled')
                or bbot:GetValue('Hit Boxes', 'Enabled')
            then
                local ontouch = bulletData.ontouch
                local extra = bulletData.extra

                bulletData.ontouch = function(
                    self,
                    part,
                    position,
                    normal,
                    exit,
                    exitnorm
                ) -- goated hitbox method
                    if not ticketCache[extra.bulletTicket] then
                        if
                            bbot:GetValue('Hit Boxes', 'Enabled')
                            and part:IsDescendantOf(hitboxObjects)
                        then
                            ...
                            send(
                                ...
                                players[part.Name],
                                position,
                                bbot:GetValue('Hit Boxes', 'Hit Part'),
                                extra.bulletTicket + ticketAddition,
                                network.getTime()
                                    + newSpawnCache.latency
                                    + newSpawnCache.currentAddition
                            )
                        elseif
                            bbot:GetValue('Backtracking', 'Enabled')
                            and part:IsDescendantOf(backtrackObjects)
                        then
                            local model = part

                            while
                                model.ClassName ~= 'Model'
                                or model.Parent.ClassName ~= 'Folder'
                            do
                                model = model.Parent
                            end
                            ...
                            local head = entry._thirdPersonObject
                                and entry._thirdPersonObject._characterModelHash
                                and entry._thirdPersonObject._characterModelHash.Head
    ...
    function hudScopeInterface.updateScope(...)
        if bbot:GetValue('Gun Mods', 'No Sniper Scope') then -- more scripts need this
    ...
    function recoil.applyImpulse(...)
        if aimbotting or bbot:GetValue('Gun Mods', 'No Recoil') then
            return
        end
    ...
    function firearmObject:reload()
        if
            bbot:GetValue('Gun Mods', 'Instant Reload')
            and self._spareCount > 0
        then
            if self._spareCount >= self._weaponData.magsize then
                self._spareCount = self._spareCount
                    - (self._weaponData.magsize - self._magCount)
    ...
    function firearmObject:computeWalkSway(dy, dx)
        if bbot:GetValue('Gun Mods', 'No Walk Sway') or aimbotting then
            dy = 0
            dx = 0
        end
    ...
    function firearmObject.computeGunSway(...)
        if bbot:GetValue('Gun Mods', 'No Gun Sway') or aimbotting then
    ...
    function cframeLib.fromAxisAngle(x, y, z)
        if aimbotting then -- or bbot:GetValue("Gun Mods", "No Camera Sway") then
            local controller = weaponInterface.getActiveWeaponController()
            local weapon = controller and controller:getActiveWeapon()

            return (weapon and weapon._blackScoped and CFrame.identity)
                or fromAxisAngle(x, y, z)
        end
    ...
    function modifyData.getModifiedData(data, ...)
        ...
        if bbot:GetValue('Gun Mods', 'No Spread') then
            data.hipfirespread = 0
            data.hipfirestability = 99999
            data.hipfirespreadrecover = 99999
        end

        if bbot:GetValue('Gun Mods', 'Small Crosshair') then
            data.crosssize = 10
        ...
        if bbot:GetValue('Gun Mods', 'No Crosshair') then
                ...
                then
                    for slot, name in weapons do
                        ...
                        if name == displayname then
                            local realData = contentDatabase.getWeaponData(
                                realWeapons[class][slot]
                            )
                            local firecap = realData.firecap
                                or (
                                    (
                                        realData.variablefirerate
                                        and math.max(
                                            table.unpack(realData.firerate)
                                        )
                                    )
                                    or realData.firerate
                                )
                            ...
                            if data.variablefirerate then
                                ...
                                for firerateIndex, firerate in data.firerate do
                                    newFireRates[firerateIndex] =
                                        math.min(firerate, firecap)
                                end
                            ...
                            if data.magsize > realData.magsize then
                                ...
                                data.sparerounds = (
                                    realData.magsize + realData.sparerounds
                                )
                                    - data.magsize
                            end
    ...
    cameraObject.step = LPH_NO_VIRTUALIZE(function(self, dt)
        if aimbotting or bbot:GetValue('Gun Mods', 'No Camera Sway') then
            mainStep(self, 0)
            self._lookDt = dt
        end

        if bbot:GetValue('Gun Mods', 'No Camera Bob') then
        ...
        if
            aimbotting
            or bbot:GetValue('Gun Mods', 'No Camera Sway')
            or bbot:GetValue('Gun Mods', 'No Camera Bob')
        then
            return
        end
    ...
    debug.setupvalue(cameraObject.step, 2, {fromAxisAngle = function(...)
        return (aimbotting or bbot:GetValue("Gun Mods", "No Camera Sway")) and CFrame.identity or cframeLib.fromAxisAngle(...)
    ...
    local weaponFolder =
        game:GetService('ReplicatedStorage').Content.ProductionContent.WeaponDatabase
    ...
    function playerDataUtils.ownsWeapon(player, wepName)
        ...
        if unlockKnives then
            for i = 1, 4 do
                local index = (i == 1 and 'ONE HAND BLUNT')
                    or (i == 2 and 'ONE HAND BLADE')
                    or (i == 3 and 'TWO HAND BLUNT')
                    or 'TWO HAND BLADE'

                if
                    weaponFolder[index]:FindFirstChild(string.upper(wepName))
                then
                    return true
                end
            end
    ...
    function audioSystem.playSoundId(
        assetId,
        priority,
        volume,
        pitch,
        ...
        if bbot:GetValue('Sounds', 'Shoot Sound') ~= 'None' then
            local controller = weaponInterface.getActiveWeaponController()
            local weapon = controller and controller:getActiveWeapon()

            if weapon and assetId == weapon:getWeaponStat('firesoundid') then
                return playSoundId(
                    customAudios[bbot:GetValue('Sounds', 'Shoot Sound')],
                    priority,
                    volume
                )
            end
    ...
    function audioSystem.playSound(soundName, ...)
        ...
        if
            bbot:GetValue('Sounds', 'Hit Sound') ~= 'None'
            and soundName == 'hitmarker'
        then
            return playSoundId(
                customAudios[bbot:GetValue('Sounds', 'Hit Sound')],
                1,
                args[3]
            )
        elseif
            bbot:GetValue('Sounds', 'Footstep Sound') ~= 'None'
            and (args[1] == 'SelfFoley')
        then
            return playSoundId(
                customAudios[bbot:GetValue('Sounds', 'Footstep Sound')],
                args[2],
                args[3]
            )
        elseif
            bbot:GetValue('Sounds', 'Kill Sound') ~= 'None'
            and (soundName == 'killshot' or soundName == 'headshotkill')
        then
            return playSoundId(
                customAudios[bbot:GetValue('Sounds', 'Kill Sound')],
                1,
                args[3]
            )
        elseif
            bbot:GetValue('Sounds', 'Got Hit Sound') ~= 'None'
            and (soundName == 'crackSmall' or soundName == 'crackBig')
        then
            return playSoundId(
                customAudios[bbot:GetValue('Sounds', 'Got Hit Sound')],
                1,
                args[3]
            )
        end
    ...
    -- Crashes game. i dont know how im gonna find a new method for this one
    ...
    function effects.breakwindow(part, receiveWindow, netTime)
        if part.Name ~= 'Window' then
            return
        elseif bbot:GetValue('Sounds', 'Glass Breaking Sound') ~= 'None' then
            misc.ChildAdded:Connect(function(child)
                if
                    child.ClassName == 'Part'
                    and child.CFrame == part.CFrame
                then
                    child.ChildAdded:Connect(function(sound)
                        if sound.ClassName == 'Sound' then
                            sound.SoundId = customAudios[bbot:GetValue(
                                'Sounds',
    ...
    function charObject:setBaseWalkSpeed(speed)
        ...
        return setBaseWalkSpeed(
            self,
            bbot:GetValue('Movement', 'Walk Speed')
                    and bbot:GetValue('Movement', 'Set Speed')
                or speed
        )
    end
    ...
    function charObject:jump(height, vaulting)
        return jump(
            ...
            4
                + (
                    bbot:GetValue('Movement', 'Jump Power')
                        and bbot:GetValue('Movement', 'Height Addition')
                    or 0
                ),
            vaulting
        )
    ...
    callbackList['Movement%%Walk Speed'] = function(state)
        if charInterface.isAlive() then
            ...
            if state then
                setBaseWalkSpeed(
                    object,
                    bbot:GetValue('Movement', 'Set Speed')
                )
            else
    ...
    callbackList['Tweaks%%Custom Kill Notification'] = function(state)
        hudnotify.typeList.kill[1] = state
                and bbot:GetValue('Tweaks', 'Notification Text')
            or 'Enemy Killed!'
    end

    callbackList['Tweaks%%Notification Text'] = function(state)
        if bbot:GetValue('Tweaks', 'Custom Kill Notification') then
            hudnotify.typeList.kill[1] = state
        end
    end
    ...
    for i, v in getgc(true) do
        if
            type(v) == 'table'
            and rawget(v, 'Mentha Spicata')
            and rawget(v, 'Dove blue')
        then
            camoDatabase = v
            break
        end
        ...
        for camoName, camoData in camoDatabase do
            if camoData.Case then
                playerDataUtils.getCasePacketData(
                    ...
                    { -- unlocks a bunch of knives as camos if u scroll through the camos
                        ALL = true,
                    }
            end
        end
    ...
    callbackList['Tweaks%%Unlock All'] = function() -- Goated client side unlock all
        local classData =
            playerDataUtils.getClassData(playerClient.getPlayerData())
    ...
    callbackList['Anti Aim%%Enabled (May Cause Despawning)'] = function(state)
        ...
        if charInterface.isAlive() then
            if
                bbot:GetValue('Third Person', 'Show Character')
                and bbot:GetValue(
                    'Third Person',
                    'Apply Anti Aim To Character'
                )
                and currentObj
            then
                if bbot:GetValue('Anti Aim', 'Force Stance') then
                    local stance = state
                            and bbot:GetValue('Anti Aim', 'Set Stance')
                        or newSpawnCache.stance
                        or 'stand'
                    currentObj:setStance(stance)
                end

                if bbot:GetValue('Anti Aim', 'Jitter') then
            ...
            if bbot:GetValue('Anti Aim', 'Jitter') and not state then
    ...
    callbackList['Movement%%Noclip'] = function(state) -- Noclip, yeah im not fixing this
        if charInterface.isAlive() and not state then
            charInterface.getCharacterObject():getRealRootPart().CanCollide =
                true
        end
    end
    ...
    callbackList['World Visuals%%Ambient'] = function(state)
        if not state then
            if charInterface.isAlive() then
                ...
                local outdoorAmbient =
                    lighting.MapLighting:FindFirstChild('OutdoorAmbient')
    ...
    --    if bbot:GetValue("Movement", "Noclip") and ref.ClassName == "Model" then
    ...
    local function updateCrosshair()
        local enabled = bbot:GetValue('Crosshair', 'Enabled')
        crossdot.Visible = enabled and bbot:GetValue('Crosshair', 'Show Dot')
        ...
        if not bbot:GetValue('Crosshair', 'Rainbow Crosshair') then
            local color = bbot:GetValue('Crosshair', 'Crosshair Color')
    ...
    local function updateCrosshairPos(force) -- i didnt want to make this
        local barrel = bbot:GetValue('Crosshair', 'Follow Recoil')
            and getBarrelLocation()
        if barrel then
            barrel = (barrel.Z > 0 and Vector2.new(barrel.X, barrel.Y))
        end
        local middle = barrel or (camera.ViewportSize * 0.5)
        local x, y = middle.X, middle.Y
        local sx = bbot:GetValue('Crosshair', 'X Space') * 0.5
        local sy = bbot:GetValue('Crosshair', 'Y Space') * 0.5
        local w = bbot:GetValue('Crosshair', 'X Size')
        local h = bbot:GetValue('Crosshair', 'Y Size')
        local speed = bbot:GetValue('Crosshair', 'Spin Speed')
    ...
    callbackList['Crosshair%%Spin Speed'] = function(state)
        updateCrosshairPos(state == 0)
    end
    ...
    table.insert(
        ...
        camera.ChildAdded:Connect(function(model)
            if model.ClassName == 'Model' then
                ...
                if bbot:GetValue('Chams', prefix .. 'Chams') then
                    local properties, uncache = cham.new(model, {
                        Material = Enum.Material[bbot:GetValue(
                            'Chams',
                            prefix .. 'Material'
                        )],
                        Transparency = bbot:GetValue(
                            'Chams',
                            prefix .. 'Transparency'
                        ) * 0.01,
                        Color = bbot:GetValue('Chams', prefix .. 'Color'),
                    }, false, true, false)
    ...
    callbackList['Server Hopper%%Clear Cached Servers'] = function()
        writefile(
            folderName .. '/cache/servers.json',
            httpService:JSONEncode({})
        )
    end

    local function hopServers()
        local cachedServers = httpService:JSONDecode(
            readfile(folderName .. '/cache/servers.json')
        )
        ...
        do
            if
                ...
                and v.id ~= game.JobId
                and not table.find(cachedServers, v.id)
            then
                table.insert(cachedServers, v.id)
                writefile(
                    folderName .. '/cache/servers.json',
                    httpService:JSONEncode(cachedServers)
                )
    ...
    function networkConnections.startvotekick(username, delay, votes)
        if
            bbot:GetValue('Server Hopper', 'Server Hop On Votekick')
            and username == localPlayer.Name
    ...
    local function chatSpam() -- dunno why this doesnt work on some executors

        if bbot:GetValue('Chat Spam', 'Enabled') then
            local list = chatSpamLists[bbot:GetValue('Chat Spam', 'Spam List')]
            local newSpamIndex = 1

            if #list ~= 1 then
                repeat
                    newSpamIndex = math.random(1, #list)
                until newSpamIndex ~= lastSpamIndex
            end

            --network:send("sendChatMessage", list[newSpamIndex], bbot:GetValue("Chat Spam", "Team Chat"))
            globalChannel:SendAsync(list[newSpamIndex])
            lastSpamIndex = newSpamIndex
        end

        task.delay(bbot:GetValue('Chat Spam', 'Spam Delay'), chatSpam)
    end
    ...
    local function initKnifeBot()
        ...
        while raging do
            ...
            if root then
                ...
                if
                    newSpawnCache.spawned
                    and newSpawnCache.lastUpdate
                    and (clock - newSpawnCache.spawnTime) > 1
                    and (nextScan - clock) <= 0
                    and not roundSystem.roundLock
                    and (
                        (
                            not bbot:GetValue(
                                'Knife Bot',
                        ...
                        ) or (newSpawnCache.slot == 3)
                    )
                then
                    local closestCharacters = getClosestPlayers(
                        newSpawnCache.lastUpdate,
                        true,
                        bbot:GetValue('Knife Bot', 'Only Kill Target Status'),
                        bbot:GetValue('Knife Bot', 'Whitelist Friendly Status')
                    )

                    if closestCharacters then
                        for entryIndex = 1, #closestCharacters do
                            local position =
                                closestCharacters[entryIndex]._receivedPosition
                            local targetPlayer =
                                closestCharacters[entryIndex]._player
                            ...
                            if position then
                                ...
                                if result == true then
                                    local path = pathfinding.optimizePath(
                                        data.waypoints,
                                        9.9
                                    )
                                    ...
                                    if not TPfailed then
                                        ...
                                        task.delay(
                                            killCooldownMS * 0.001,
                                            function()
                                                killedPlayers[targetPlayer] =
                                                    false
                                            end
                                        )
                                    end
    ...
    callbackList['Knife Bot%%Kill All (May Despawn)'] = function(state)
        ...
        if charInterface.isAlive() then
            if
                not bbot:GetValue('Knife Bot', 'Only When Holding Knife')
                and newSpawnCache.slot ~= 3
            then
                if state then
                    send(
                        network,
    ...
    local espInterface = loadstring(
        game:HttpGet(
            'https://raw.githubusercontent.com/jensonhirst/Sirius/refs/heads/request/library/sense/source.lua'
        )
    )()
    ...
    espInterface.getCharacter = LPH_NO_VIRTUALIZE(function(player)
        ...
        local thirdPerson = playerReplicationObject:isReady()
            and playerReplicationObject._smoothReplication._prevFrameTime
            and playerReplicationObject
            and playerReplicationObject:getThirdPersonObject()
        return thirdPerson and thirdPerson:getCharacterModel(),
            thirdPerson and thirdPerson:getRootPart()
    end)
    ...
    callbackList['Team ESP%%Enabled'] = function(state) -- why doesnt this work bruh
    ...
    callbackList['Custom Model%%Asset ID'] = function(state)
        if bbot:GetValue('Custom Model', 'Custom Character Model') then
            if customModel then
                customModel.Parent = nil
            end

            if state then
                local modelId = 'rbxassetid://'
                    .. string.gsub(state, 'rbxassetid://', '')
                customModel = game:GetObjects(modelId)

                if
                    not customModel
                    or not customModel[1]
                    or (type(customModel[1]) ~= 'userdata')
                then
                    customModel = nil
                else
                    customModel = customModel[1]
                    local part = customModel.ClassName == 'Model'
                            and customModel.PrimaryPart
                        or customModel
                    part.Anchored = true
                    part.CanCollide = false
    ...
    callbackList['Custom Model%%Asset ID'](
        bbot:GetValue('Custom Model', 'Asset ID')
    )
    ...
    table.insert(
        connectionList,
        runService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function(ndt)
            ...
            if customModel and rootPart then
                ...
                part.CFrame = rootPart.CFrame
                    * CFrame.new(
                        bbot:GetValue('Custom Model', 'Asset Offset X'),
                        bbot:GetValue('Custom Model', 'Asset Offset Y'),
                        bbot:GetValue('Custom Model', 'Asset Offset Z')
                    )
            end
                    ...
                    if currentObj or started then
                        if started then
                            ...
                            if
                                bbot:GetValue(
                                    'More Chams',
                                    'Third Person Character Chams'
                                )
                            then
                                local _, uncache =
                                    cham.new(currentObj._character, {
                                        Transparency = bbot:GetValue(
                                            'More Chams',
                            ...
                            if
                                ...
                                and bbot:GetValue('Anti Aim', 'Force Stance')
                                and bbot:GetValue(
                                    'Third Person',
                                    'Apply Anti Aim To Character'
                                )
                                and currentObj
                            then
                                currentObj:setStance(
                                    string.lower(
                                        bbot:GetValue('Anti Aim', 'Set Stance')
                                    )
                                )
                            end
                        end
                        ...
                        local angles =
                            cameraInterface:getActiveCamera():getAngles()
                        if
                            bbot:GetValue(
                                'Anti Aim',
                                'Enabled (May Cause Despawning)'
                        ...
                        fakeRepObject._lookangles.t = angles -- incase interpolation breaks on smooth replication we will still have our third person
                        fakeRepObject._lookangles.p = angles

                        fakeRepObject._smoothReplication:receive(
                            clockTime,
                            tickTime,
                            {
                                t = tickTime,
            ...
            if
                bbot:GetValue('Rage Bot', 'Enabled')
                and clockTime > nextShot
                and not roundSystem.roundLock
                and not bbot:GetValue('Knife Bot', 'Kill All (May Despawn)')
            then --  and newSpawnCache.hasPinged
                ...
                then
                    ...
                    local closestPlayers = getClosestPlayers(
                        origin,
                        false,
                        bbot:GetValue('Rage Bot', 'Only Shoot Target Status'),
                        bbot:GetValue('Rage Bot', 'Whitelist Friendly Status')
                    )
                    ...
                    if
                        ...
                        and penetration
                        and speed
                        and (weapon._magCount > 0 or weapon._spareCount > 0)
                    then
                        for playerIndex = 1, #closestPlayers do
                            local entry = closestPlayers[playerIndex]
                            local newOrigin, newTarget, velocity, hitTime =
                                scanPositions(
                                    origin,
                                    entry._receivedPosition,
                                    publicSettings.bulletAcceleration,
                            ...
                            if newOrigin then
                                if weapon._magCount < 1 then
                                    if weapon._spareCount >= data.magsize then
                                        ...
                                        weapon._spareCount = weapon._spareCount
                                            - weapon._magCount
                                    else
                                ...
                                if
                                    bbot:GetValue('Rage Bot', 'Shoot Effects')
                                    and weapon._barrelPart
                                then
                                    ...
                                    if not weapon._aiming then
                                        crosshairsInterface.fireImpulse(
                                            data.crossexpansion
                                        )
                                    end
                                ...
                                local fireDelay = 60
                                    / (
                                        data.variablefirerate
                                            and data.firerate[weapon._firemodeIndex]
                                        or data.firerate
                                    )

                                --if bbot:GetValue("Rage Bot", "Firerate (May Cause Kicking)") then
    ...
    table.insert(
        connectionList,
        runService.Stepped:Connect(function(time, ndt)
            local currentCharObject = charInterface.getCharacterObject()
            local rootPart = currentCharObject
                and currentCharObject:getRealRootPart()
            local clockTime = os.clock()
            ...
            if customModel and rootPart then
                local part = customModel.ClassName == 'Model'
                        and customModel.PrimaryPart
                    or customModel
                customModel.CFrame = rootPart.CFrame
                    * CFrame.new(
                        bbot:GetValue('Custom Model', 'Asset Offset X'),
                        bbot:GetValue('Custom Model', 'Asset Offset Y'),
                        bbot:GetValue('Custom Model', 'Asset Offset Z')
            ...
            replicationInterface.operateOnAllEntries(function(player, entry)
                if player.Team ~= localplayer.Team then
                    local character = entry._thirdPersonObject
                        and entry._thirdPersonObject._characterModelHash
                    movementCache.position[player] = movementCache.position[player]
                        or {}

                    if character then
                        table.insert(
                            movementCache.position[player],
                            1,
            ...
            if
                bbot:GetValue('Anti Aim', 'Enabled (May Cause Despawning)')
                and bbot:GetValue('Anti Aim', 'Jitter')
                and rootPart
                and (clockTime - lastJitter)
                    > (1 / bbot:GetValue('Anti Aim', 'Jitter Speed') / 2)
            then
            ...
            if
                bbot:GetValue('Movement', 'Bunny Hop')
                and rootPart
                and (
                    not bbot:GetValue('Movement', 'Only While Jumping')
                    or userInputService:IsKeyDown(Enum.KeyCode.Space)
                )
            then
                currentCharObject._lastJumpTime = 0
                currentCharObject:jump(
                    4
                        + (
                            bbot:GetValue('Movement', 'Jump Power')
                                and bbot:GetValue(
            ...
            if bbot:GetValue('Movement', 'Noclip') and rootPart then
            ...
            --[[
        if false then --bbot:GetValue("Movement", "Fly") and rootPart then
            ...
            if direction == Vector3.zero then
                ...
                rootPart.Anchored = false
                rootPart.Velocity = direction.Unit * bbot:GetValue("Movement", "Fly Speed")
            end
        end
        ]]

            if bbot:GetValue('Hit Boxes', 'Enabled') then
                replicationInterface.operateOnAllEntries(function(player, entry)
                    if player.Team ~= localplayer.Team then
                        ...
                        if entry._receivedPosition then
                            if not sphere then
                                local size = bbot:GetValue('Hit Boxes', 'Size')
                                sphere = Instance.new('Part')
                                sphere.Name = player.Name
                                sphere.CanCollide = true
                                sphere.Shape = Enum.PartType.Ball
                                sphere.Size = Vector3.new(size, size, size)
                                sphere.Material = Enum.Material[bbot:GetValue(
                                    'Hit Boxes',
                                    'Material'
                                )]
                                sphere.Transparency = bbot:GetValue(
                                    'Hit Boxes',
                                    'Transparency'
                                ) * 0.01
                                sphere.Color =
                                    bbot:GetValue('Hit Boxes', 'Color')
            ...
            if bbot:GetValue('Backtracking', 'Enabled') then
                local delay = 1 / bbot:GetValue('Backtracking', 'Refresh Rate')

                if clockTime > backtrackTime + delay then
                    replicationInterface.operateOnAllEntries(
                        function(player, entry)
                            local entryThirdPersonObject =
                                entry._thirdPersonObject
                            local character = entryThirdPersonObject
                                and entryThirdPersonObject._character

                            if
                                player.Team ~= localplayer.Team and character
                            then
                                local clone
                                ...
                                then
                                    ...
                                    part.CFrame =
                                        entryThirdPersonObject._rootPart.CFrame
    ...
    table.insert(
        ...
        runService.RenderStepped:Connect(LPH_NO_VIRTUALIZE(function(deltaTime)
            ...
            aimbotting = false
            if bbot:GetValue('Aim Bot', 'Enabled') and aiming then
                local target, entry, part = getClosest(
                    aimbotfov.Position,
                    bbot:GetValue('Aim Bot', 'Use FOV') and aimbotfov.Radius,
                    bbot:GetValue('Aim Bot', 'Use Dead FOV')
                        and aimbotdeadfov.Radius,
                    bbot:GetValue('Aim Bot', 'Visible Check'),
                    bbot:GetValue('Aim Bot', 'Target Part')
                )
                ...
                if target and movementCache.position[entry._player][15] then
                    ...
                    local x = vx > cameraObj._maxAngle and cameraObj._maxAngle
                        or vx < cameraObj._minAngle and cameraObj._minAngle
                        or vx
                    local y = (vy + pi - cy) % tau - pi + cy
                    local newangles = Vector3.new(x, y, 0)
                    local smoothing = bbot:GetValue('Aim Bot', 'Smoothness')

                    if smoothing ~= 0 then
                        newangles = cameraObj._angles:lerp(
                            newangles,
                            math.clamp(
                                1 - smoothing + (clockTime - aimTime) ^ 2,
                                0,
                                1
                            )
                        )
                    ...
                    cameraObj._delta = (newangles - cameraObj._angles)
                        / deltaTime
            ...
            local circlePos
            if bbot:GetValue('FOV Settings', 'FOV Follows Recoil') then
            ...
            if bbot:GetValue('FOV Settings', 'Dynamic FOV') then
                local factor = not charInterface.isAlive() and 1
                    or (
                        cameraInterface.getActiveCamera():getBaseFov()
                        / camera.FieldOfView
                    )
                aimbotfov.Radius = bbot:GetValue('Aim Bot', 'FOV Radius')
                    * factor
                aimbotdeadfov.Radius = bbot:GetValue(
                    'Aim Bot',
                    'Dead FOV Radius'
                ) * factor
                silentaimfov.Radius = bbot:GetValue('Silent Aim', 'FOV Radius')
                    * factor
                silentaimdeadfov.Radius = bbot:GetValue(
                    'Silent Aim',
                    'Dead FOV Radius'
                ) * factor
            end

            if bbot:GetValue('World Visuals', 'Ambient') then
                local color = bbot:GetValue('World Visuals', 'Ambient Color')
                lighting.Ambient = color
                lighting.OutdoorAmbient = color
            end

            if bbot:GetValue('Crosshair', 'Enabled') then
                if
                    (bbot:GetValue('Crosshair', 'Spin Speed') > 0)
                    or bbot:GetValue('Crosshair', 'Follow Recoil')
                then
                    updateCrosshairPos()
                end

                if bbot:GetValue('Crosshair', 'Rainbow Crosshair') then
                    local rainbow = Color3.fromHSV(
                        (
                            clockTime
                            * bbot:GetValue('Crosshair', 'Rainbow Speed')
                        ) % 1,
                        1,
                        1
                    )
    ...
    unloadMain = function() -- not finished
        network.send = send
...
LPH_NO_VIRTUALIZE(function() -- Make UI
    ...
    task.spawn(function()
        while stillGoing do
            ...
            for _, name in listfiles(folderName .. '/sounds') do
                --name = string.gsub(string.gsub(string.gsub(name, folderName .. "/sounds", ""), folderName .. [[\\]], ""), folderName .. "/", "")
                local fileNames = string.split(name, 'sounds')
                name = string.sub(fileNames[2], 2, string.len(name))
                table.insert(soundFileList, name)

                if
                    not customAudios[name]
                    and pcall(getcustomasset, folderName .. '/sounds/' .. name)
                then
                    customAudios[name] =
                        getcustomasset(folderName .. '/sounds/' .. name)
                end
            end
            ...
            for _, name in listfiles(folderName .. '/chat spam lists') do
                name = string.gsub(
                    string.gsub(name, folderName .. '/chat spam lists/', ''),
                    folderName .. '\\chat spam lists\\',
                    ''
                )
                table.insert(chatListsFiles, name)

                if not chatSpamLists[name] then
                    chatSpamLists[name] = httpService:JSONDecode(
                        readfile(folderName .. '/chat spam lists/' .. name)
                    )
                end
            end
    ...
    if not isfile(folderName .. '/chat spam lists/default.txt') then
        writefile(
            folderName .. '/chat spam lists/default.txt',
            httpService:JSONEncode({
    ...
    if not configExists then
        writefile(
            folderName .. '/cache/lastfile.json',
            httpService:JSONEncode({})
        )
    end
    ...
    if not cacheExists then
        writefile(
            folderName .. '/cache/settings.json',
            httpService:JSONEncode({})
        )
    else
        uiCache = httpService:JSONDecode(
            readfile(folderName .. '/cache/settings.json')
        )
    end
    ...
    if isfile(folderName .. '/theme.json') then
        ...
        bbot.theme = {
            accent = Color3.fromRGB(
                table.unpack(userThemeData['Accent Color'])
            ),
            text = Color3.fromRGB(table.unpack(userThemeData['Text Color'])),
            background = Color3.fromRGB(
                table.unpack(userThemeData['Background Color'])
            ),
            lightbackground = Color3.fromRGB(
                table.unpack(userThemeData['Light Background Color'])
            ),
            hidden = Color3.fromRGB(
                table.unpack(userThemeData['Hidden Color'])
            ),
            hiddenText = Color3.fromRGB(
                table.unpack(userThemeData['Hidden Text Color'])
            ),
            outline = Color3.fromRGB(
                table.unpack(userThemeData['Outline Color'])
            ),
        }
    else
        local themeData = {
        ...
        writefile(
            folderName .. '/theme.json',
            httpService:JSONEncode(themeData)
        )
    end

    local menu = bbot:CreateMenu(
        title or defaultUIName,
        (not cacheExists) or devMode or uiCache.open,
        cacheExists and uiCache.index or 1
    )
    bbot.GetValue = menu.GetValue
    bbot.sectionIndexes = menu.sectionIndexes
    ...
    local function getCallback(name)
        return function(value)
            if value ~= nil then
                local oldConfig = httpService:JSONDecode(
                    readfile(folderName .. '/cache/lastfile.json')
                )
                local keybinds = {}
    ...
    menu.updateCache = function()
        saveConfig(
            folderName .. '/cache/settings.json',
            { open = menu.open, index = menu.tabIndex }
        )
    end
    ...
    aimbot
        :AddToggle('Enabled', false, getCallback('Aim Bot%%Enabled'))
        :AddKeyBind(nil, 'Key Bind')
    aimbot:AddToggle(
        'Visible Check',
        false,
        getCallback('Aim Bot%%Visible Check')
    )
    aimbot:AddSlider(
        'Smoothness',
        0,
        ...
        0.01,
        'x',
        getCallback('Aim Bot%%Smoothness')
    )
    aimbot:AddDropdown(
        'Target Part',
        'Head',
        { 'Head', 'Torso' },
        getCallback('Aim Bot%%Target Part')
    )
    ...
    aimbot:AddSlider(
        ...
        1,
        'px',
        getCallback('Aim Bot%%FOV Radius')
    )
    aimbot
        :AddToggle(
            'Show FOV Circle',
            false,
            getCallback('Aim Bot%%Show FOV Circle')
        )
        :AddKeyBind(nil, 'FOV Key Bind')
        :AddColorPicker(
            'FOV Circle Color',
            Color3.new(1, 1, 1),
            getCallback('Aim Bot%%FOV Circle Color')
        )
    aimbot:AddToggle(
        'Use Dead FOV',
        false,
        getCallback('Aim Bot%%Use Dead FOV')
    )
    aimbot:AddSlider(
        'Dead FOV Radius',
        100,
        ...
        1,
        'px',
        getCallback('Aim Bot%%Dead FOV Radius')
    )
    aimbot
        :AddToggle(
            'Show Dead FOV Circle',
            false,
            getCallback('Aim Bot%%Show Dead FOV Circle')
        )
        :AddKeyBind(nil, 'Dead FOV Key Bind')
        :AddColorPicker(
            'Dead FOV Circle Color',
            Color3.new(1, 1, 1),
            getCallback('Aim Bot%%Dead FOV Circle Color')
        )

    fovsettings:AddToggle(
        'FOV Follows Recoil',
        false,
        getCallback('FOV Settings%%FOV Follows Recoil')
    )
    fovsettings:AddToggle(
        'Dynamic FOV',
        false,
        getCallback('FOV Settings%%Dynamic FOV')
    )
    --fovsettings:AddSlider("Circle Side Number", 48, 3, 64, 1, "", getCallback("FOV Settings%%Circle Side Number"))
    fovsettings:AddSlider(
        'Circle Opacity',
        100,
        1,
        100,
        1,
        '%',
        getCallback('FOV Settings%%Circle Opacity')
    )
    fovsettings:AddToggle(
        'Fill Circles',
        false,
        getCallback('FOV Settings%%Fill Circles')
    )

    silentaim
        :AddToggle('Enabled', false, getCallback('Silent Aim%%Enabled'))
        :AddKeyBind(nil, 'Key Bind')
    silentaim:AddToggle(
        'Visible Check',
        false,
        getCallback('Silent Aim%%Visible Check')
    )
    ...
    silentaim:AddSlider(
        ...
        1,
        '%',
        getCallback('Silent Aim%%Hit Chance')
    )
    silentaim:AddSlider(
        'Head Shot Chance',
        100,
        ...
        1,
        '%',
        getCallback('Silent Aim%%Head Shot Chance')
    )
    ...
    silentaim:AddSlider(
        ...
        1,
        'px',
        getCallback('Silent Aim%%FOV Radius')
    )
    silentaim
        :AddToggle(
            'Show FOV Circle',
            false,
            getCallback('Silent Aim%%Show FOV Circle')
        )
        :AddKeyBind(nil, 'FOV Key Bind')
        :AddColorPicker(
            'FOV Circle Color',
            Color3.new(1, 1, 1),
            getCallback('Silent Aim%%FOV Circle Color')
        )
    silentaim:AddToggle(
        'Use Dead FOV',
        false,
        getCallback('Silent Aim%%Use Dead FOV')
    )
    silentaim:AddSlider(
        'Dead FOV Radius',
        100,
        ...
        1,
        'px',
        getCallback('Silent Aim%%Dead FOV Radius')
    )
    silentaim
        :AddToggle(
            'Show Dead FOV Circle',
            false,
            getCallback('Silent Aim%%Show Dead FOV Circle')
        )
        :AddKeyBind(nil, 'Dead FOV Key Bind')
        :AddColorPicker(
            'Dead FOV Circle Color',
            Color3.new(1, 1, 1),
            getCallback('Silent Aim%%Dead FOV Circle Color')
        )

    hitboxes
        :AddToggle('Enabled', false, getCallback('Hit Boxes%%Enabled'))
        :AddKeyBind(nil, 'Key Bind')
        :AddColorPicker(
            'Color',
            Color3.new(0.1, 0.1, 1),
            getCallback('Hit Boxes%%Color')
        )
    hitboxes:AddDropdown(
        'Hit Part',
        'Head',
        { 'Head', 'Torso' },
        getCallback('Hit Boxes%%Hit Part')
    )
    hitboxes:AddSlider(
        'Size',
        20,
    ...
    hitboxes:AddSlider(
        ...
        1,
        '%',
        getCallback('Hit Boxes%%Transparency')
    )
    hitboxes:AddDropdown(
        'Material',
        'SmoothPlastic',
        { 'ForceField', 'SmoothPlastic', 'Glass' },
        getCallback('Hit Boxes%%Material')
    )

    backtrack
        :AddToggle('Enabled', false, getCallback('Backtracking%%Enabled'))
        :AddKeyBind(nil, 'Key Bind')
        :AddColorPicker(
            'Character Color',
            Color3.new(0.1, 0.1, 1),
            getCallback('Backtracking%%Characters Color')
        )
    backtrack:AddSlider(
        'Refresh Rate',
        2,
        ...
        1,
        ' Characters/Second',
        getCallback('Backtracking%%Refresh Rate')
    )
    backtrack:AddSlider(
        'Character Duration',
        1,
        ...
        0.1,
        ' Seconds',
        getCallback('Backtracking%%Character Duration')
    )
    backtrack:AddSlider(
        'Character Transparency',
        50,
        ...
        1,
        '%',
        getCallback('Backtracking%%Character Transparency')
    )
    backtrack:AddDropdown(
        'Character Material',
        'ForceField',
        { 'ForceField', 'SmoothPlastic', 'Glass' },
        getCallback('Backtracking%%Character Material')
    )
    backtrack:AddToggle(
        'Clone Character',
        true,
        getCallback('Backtracking%%Clone Character')
    )
    ...
    gunmods:AddToggle(
        'Small Crosshair',
        false,
        getCallback('Gun Mods%%Small Crosshair')
    )
    gunmods:AddToggle(
        'No Crosshair',
        false,
        getCallback('Gun Mods%%No Crosshair')
    )
    gunmods:AddToggle(
        'No Sniper Scope',
        false,
        getCallback('Gun Mods%%No Sniper Scope')
    )
    gunmods:AddToggle(
        'No Camera Sway',
        false,
        getCallback('Gun Mods%%No Camera Sway')
    )
    gunmods:AddToggle(
        'No Camera Bob',
        false,
        getCallback('Gun Mods%%No Camera Bob')
    )
    gunmods:AddToggle(
        'No Walk Sway',
        false,
        getCallback('Gun Mods%%No Walk Sway')
    )
    gunmods:AddToggle(
        'No Gun Sway',
        false,
        getCallback('Gun Mods%%No Gun Sway')
    )
    gunmods:AddToggle(
        'Instant Reload',
        false,
        getCallback('Gun Mods%%Instant Reload')
    )

    ragebot
        :AddToggle('Enabled', false, getCallback('Rage Bot%%Enabled'))
        :AddKeyBind(nil, 'Key Bind')
    ragebot:AddToggle(
        'Shoot Effects',
        false,
        getCallback('Rage Bot%%Shoot Effects')
    )
    ragebot:AddToggle(
        'Fire Position Scanning',
        false,
        getCallback('Rage Bot%%Fire Position Scanning')
    )
    ragebot:AddSlider(
        ...
        0.1,
        ' Studs',
        getCallback('Rage Bot%%Fire Position Offset')
    )
    ragebot:AddToggle(
        'Hit Position Scanning',
        false,
        getCallback('Rage Bot%%Hit Position Scanning')
    )
    ragebot:AddSlider(
        'Hit Position Offset',
        6,
        ...
        0.1,
        ' Studs',
        getCallback('Rage Bot%%Hit Position Offset')
    )
    --ragebot:AddToggle("Firerate (May Cause Kicking)", false, getCallback("Rage Bot%%Firerate (May Cause Kicking)"))
    ragebot
        :AddToggle(
            'Only Shoot Target Status',
            false,
            getCallback('Rage Bot%%Only Shoot Target Status')
        )
        :AddKeyBind(nil, 'Target Key Bind')
    ragebot
        :AddToggle(
            'Whitelist Friendly Status',
            true,
            getCallback('Rage Bot%%Whitelist Friendly Status')
        )
        :AddKeyBind(nil, 'Friendly Key Bind')

    knifebot
        :AddToggle(
            'Kill All (May Despawn)',
            false,
            getCallback('Knife Bot%%Kill All (May Despawn)')
        )
        :AddKeyBind(nil, 'Key Bind')
    knifebot:AddToggle(
        'Only When Holding Knife',
        false,
        getCallback('Knife Bot%%Only When Holding Knife')
    )
    knifebot
        :AddToggle(
            'Only Kill Target Status',
            false,
            getCallback('Knife Bot%%Only Kill Target Status')
        )
        :AddKeyBind(nil, 'Terget Key Bind')
    knifebot
        :AddToggle(
            'Whitelist Friendly Status',
            true,
            getCallback('Knife Bot%%Whitelist Friendly Status')
        )
        :AddKeyBind(nil, 'Friendly Key Bind')
    ...
    antiaim:AddToggle(
        'Enabled (May Cause Despawning)',
        false,
        getCallback('Anti Aim%%Enabled (May Cause Despawning)')
    ...
    antiaim:AddSlider(
        ...
        1,
        ' Degrees',
        getCallback('Anti Aim%%Yaw Amount')
    )
    antiaim:AddDropdown(
        'Yaw Mode',
        'Relative',
        { 'Relative', 'Absolute' },
        getCallback('Anti Aim%%Yaw Mode')
    )
    ...
    antiaim:AddSlider(
        ...
        1,
        ' Degrees',
        getCallback('Anti Aim%%Pitch Amount')
    )
    antiaim:AddDropdown(
        'Pitch Mode',
        'Relative',
        { 'Relative', 'Absolute' },
        getCallback('Anti Aim%%Pitch Mode')
    )
    ...
    antiaim:AddSlider(
        ...
        1,
        ' Degrees/Second',
        getCallback('Anti Aim%%Spin Speed')
    )
    antiaim:AddDropdown(
        'Spin Direction',
        'Right',
        { 'Left', 'Right' },
        getCallback('Anti Aim%%Spin Direction')
    )
    ...
    antiaim:AddSlider(
        ...
        1,
        ' Shakes/Second',
        getCallback('Anti Aim%%Jitter Speed')
    )
    antiaim:AddToggle(
        'Force Stance',
        false,
        getCallback('Anti Aim%%Force Stance')
    )
    antiaim:AddDropdown(
        'Set Stance',
        'Prone',
        { 'Stand', 'Crouch', 'Prone' },
        getCallback('Anti Aim%%Set Stance')
    )

    fakelag
        :AddToggle('Fake Lag', false, getCallback('Fake Lag%%Enabled'))
        :AddKeyBind(nil, 'Key Bind')
    fakelag:AddToggle(
        'Randomize Position',
        false,
        getCallback('Fake Lag%%Randomize Position')
    )
    fakelag:AddSlider(
        'X-Axis Factor',
        0,
        ...
        1,
        ' Studs',
        getCallback('Fake Lag%%X-Axis Factor')
    )
    fakelag:AddSlider(
        'Z-Axis Factor',
        0,
        ...
        1,
        ' Studs',
        getCallback('Fake Lag%%Z-Axis Factor')
    )
    fakelag:AddSlider(
        'Refresh Distance',
        5,
        ...
        0.1,
        ' Studs',
        getCallback('Fake Lag%%Refresh Distance')
    )
    fakelag:AddSlider(
        'Refresh Rate',
        1,
        ...
        1,
        ' Seconds',
        getCallback('Fake Lag%%Refresh Rate')
    )

    enemyesp:AddToggle('Enabled', true, getCallback('Enemy ESP%%Enabled'))
    enemyesp
        :AddToggle('Boxes', false, getCallback('Enemy ESP%%Boxes'))
        :AddColorPicker(
            'Box Color',
            Color3.fromRGB(0, 255, 255),
            getCallback('Enemy ESP%%Box Color')
        )
    enemyesp:AddSlider(
        'Box Opacity',
        100,
        ...
        1,
        '%',
        getCallback('Enemy ESP%%Box Opacity')
    )
    ...
    enemyesp
        :AddToggle(
            'Box Outlines',
            false,
            getCallback('Enemy ESP%%Box Outlines')
        )
        :AddColorPicker(
            'Box Outline Color',
            Color3.fromRGB(0, 0, 0),
            getCallback('Enemy ESP%%Box Outline Color')
        )
    enemyesp:AddSlider(
        'Box Outline Opacity',
        100,
        ...
        1,
        '%',
        getCallback('Enemy ESP%%Box Outline Opacity')
    )
    --enemyesp:AddSlider("Box Outline Thickness", 5, 2, 10, 1, " px", getCallback("Enemy ESP%%Box Outline Thickness"))
    enemyesp
        :AddToggle('Fill Boxes', false, getCallback('Enemy ESP%%Fill Boxes'))
        :AddColorPicker(
            'Box Inside Color',
            Color3.fromRGB(0, 255, 255),
            getCallback('Enemy ESP%%Box Inside Color')
        )
    enemyesp:AddSlider(
        'Box Inside Opacity',
        100,
        ...
        1,
        '%',
        getCallback('Enemy ESP%%Box Inside Opacity')
    )
    --enemyesp:AddDropdown("Box Type", "Square", {"Square", "Cube"}, getCallback("Enemy ESP%%Box Type"))
    enemyesp
        :AddToggle('Health Bar', false, getCallback('Enemy ESP%%Health Bar'))
        :AddColorPicker(
            'Damage Color',
            Color3.fromRGB(255, 0, 0),
            getCallback('Enemy ESP%%Damage Color')
        )
        :AddColorPicker(
            'Health Color',
            Color3.fromRGB(0, 255, 0),
            getCallback('Enemy ESP%%Health Color')
        )
    enemyesp
        :AddToggle(
            'Health Bar Outline',
            false,
            getCallback('Enemy ESP%%Health Bar Outline')
        )
        :AddColorPicker(
            'Health Outline Color',
            Color3.fromRGB(0, 0, 0),
            getCallback('Enemy ESP%%Health Outline Color')
        )
    enemyesp
        :AddToggle('Tracers', false, getCallback('Enemy ESP%%Tracers'))
        :AddColorPicker(
            'Tracer Color',
            Color3.fromRGB(0, 255, 255),
            getCallback('Enemy ESP%%Tracer Color')
        )
    enemyesp:AddSlider(
        'Tracer Opacity',
        100,
        ...
        1,
        '%',
        getCallback('Enemy ESP%%Tracer Opacity')
    )
    ...
    enemyesp
        :AddToggle(
            'Tracer Outlines',
            false,
            getCallback('Enemy ESP%%Tracer Outlines')
        )
        :AddColorPicker(
            'Tracer Outline Color',
            Color3.fromRGB(0, 0, 0),
            getCallback('Enemy ESP%%Tracer Outline Color')
        )
    --enemyesp:AddSlider("Tracer Outlines Thickness", 4, 2, 10, 1, " px", getCallback("Enemy ESP%%Tracer Outlines Thickness"))
    enemyesp:AddSlider(
        'Tracer Outlines Opacity',
        100,
        1,
        100,
        1,
        '%',
        getCallback('Enemy ESP%%Tracer Outlines Opacity')
    )
    enemyesp:AddDropdown(
        'Tracer Origin',
        'Bottom',
        { 'Middle', 'Top', 'Bottom' },
        getCallback('Enemy ESP%%Tracer Origin')
    )
    --enemyesp:AddDropdown("Tracer Destination", "Bottom", {"Top", "Bottom"}, getCallback("Enemy ESP%%Tracer Destination"))
    enemyesp
        :AddToggle('Names', false, getCallback('Enemy ESP%%Names'))
        :AddColorPicker(
            'Names Color',
            Color3.fromRGB(255, 255, 255),
            getCallback('Enemy ESP%%Names Color')
        )
    enemyesp
        :AddToggle('Weapons', false, getCallback('Enemy ESP%%Weapons'))
        :AddColorPicker(
            'Weapons Color',
            Color3.fromRGB(255, 255, 255),
            getCallback('Enemy ESP%%Weapons Color')
        )
    enemyesp
        :AddToggle('Distances', false, getCallback('Enemy ESP%%Distances'))
        :AddColorPicker(
            'Distances Color',
            Color3.fromRGB(255, 255, 255),
            getCallback('Enemy ESP%%Distances Color')
        )
    enemyesp
        :AddToggle(
            'Health Percents',
            false,
            getCallback('Enemy ESP%%Health Percents')
        )
        :AddColorPicker(
            'Health Number Color',
            Color3.fromRGB(255, 255, 255),
            getCallback('Enemy ESP%%Health Number Color')
        )
    ...
    enemyesp
        :AddToggle(
            'Text Outlines',
            true,
            getCallback('Enemy ESP%%Text Outlines')
        )
        :AddColorPicker(
            'Text Outline Color',
            Color3.fromRGB(0, 0, 0),
            getCallback('Enemy ESP%%Text Outline Color')
        )
    enemyesp
        :AddToggle(
            'Highlight Chams',
            false,
            getCallback('Enemy ESP%%Highlight Chams')
        )
        :AddColorPicker(
            'Highlight Outline Color',
            Color3.fromRGB(0, 0, 0),
            getCallback('Enemy ESP%%Highlight Outline Color')
        )
        :AddColorPicker(
            'Highlight Fill Color',
            Color3.fromRGB(0, 0, 255),
            getCallback('Enemy ESP%%Highlight Fill Color')
        )
    enemyesp:AddSlider(
        ...
        1,
        '%',
        getCallback('Enemy ESP%%Highlight Fill Opacity')
    )
    enemyesp:AddSlider(
        ...
        1,
        '%',
        getCallback('Enemy ESP%%Highlight Outline Opacity')
    )
    enemyesp:AddToggle(
        'Highlight Visible Check',
        false,
        getCallback('Enemy ESP%%Highlight Visible Check')
    )
    ...
    chams
        :AddToggle('Arm Chams', false, getCallback('Chams%%Arm Chams'))
        :AddColorPicker(
            'Arm Color',
            Color3.new(0.1, 0.1, 1),
            getCallback('Chams%%Arm Color')
        )
    chams:AddSlider(
        'Arm Transparency',
        50,
        ...
        1,
        '%',
        getCallback('Chams%%Arm Transparency')
    )
    chams:AddDropdown(
        'Arm Material',
        'ForceField',
        { 'ForceField', 'SmoothPlastic', 'Glass' },
        getCallback('Chams%%Arm Material')
    )
    chams
        :AddToggle('Gun Chams', false, getCallback('Chams%%Gun Chams'))
        :AddColorPicker(
            'Gun Color',
            Color3.new(0.1, 0.1, 1),
            getCallback('Chams%%Gun Color')
        )
    chams:AddSlider(
        'Gun Transparency',
        50,
        ...
        1,
        '%',
        getCallback('Chams%%Gun Transparency')
    )
    chams:AddDropdown(
        'Gun Material',
        'ForceField',
        { 'ForceField', 'SmoothPlastic', 'Glass' },
        getCallback('Chams%%Gun Material')
    )
    ...
    morechams
        :AddToggle(
            'Third Person Character Chams',
            false,
            getCallback('More Chams%%Third Person Character Chams')
        )
        :AddColorPicker(
            'Character Color',
            Color3.new(0.1, 0.1, 1),
            getCallback('More Chams%%Character Color')
        )
    morechams:AddSlider(
        'Character Transparency',
        50,
        ...
        1,
        '%',
        getCallback('More Chams%%Character Transparency')
    )
    morechams:AddDropdown(
        'Character Material',
        'ForceField',
        { 'ForceField', 'SmoothPlastic', 'Glass' },
        getCallback('More Chams%%Character Material')
    )

    worldvisuals
        :AddToggle('Ambient', false, getCallback('World Visuals%%Ambient'))
        :AddKeyBind(nil, 'Ambient Key Bind')
        :AddColorPicker(
            'Ambient Color',
            Color3.new(0.1, 0.1, 1),
            getCallback('World Visuals%%Ambient Color')
        )
    worldvisuals
        :AddToggle(
            'Bullet Tracers',
            false,
            getCallback('World Visuals%%Bullet Tracers')
        )
        :AddKeyBind(nil, 'Tracers Key Bind')
        :AddColorPicker(
            'Color One',
            Color3.new(0.1, 0.1, 1),
            getCallback('World Visuals%%Color One')
        )
        :AddColorPicker(
            'Color Two',
            Color3.new(1, 0.9, 0.9),
            getCallback('World Visuals%%Color Two')
        )
    worldvisuals:AddSlider(
        ...
        0.05,
        ' Studs',
        getCallback('World Visuals%%Tracers Size')
    )
    worldvisuals:AddSlider(
        'Tracers Transparency',
        50,
        ...
        1,
        '%',
        getCallback('World Visuals%%Tracers Transparency')
    )
    worldvisuals:AddDropdown(
        'Tracers Material',
        'ForceField',
        { 'ForceField', 'SmoothPlastic', 'Glass' },
        getCallback('World Visuals%%Tracers Material')
    )
    worldvisuals
        :AddToggle(
            'Impact Points',
            false,
            getCallback('World Visuals%%Impact Points')
        )
        :AddKeyBind(nil, 'Points Key Bind')
        :AddColorPicker(
            'Points Color',
            Color3.new(0.1, 0.1, 1),
            getCallback('World Visuals%%Points Color')
        )
    worldvisuals:AddSlider(
        'Points Transparency',
        50,
        ...
        1,
        '%',
        getCallback('World Visuals%%Points Transparency')
    )
    worldvisuals:AddDropdown(
        'Points Material',
        'ForceField',
        { 'ForceField', 'SmoothPlastic', 'Glass' },
        getCallback('World Visuals%%Points Material')
    )
    worldvisuals:AddSlider(
        'Duration',
        4,
        ...
        0.5,
        ' Seconds',
        getCallback('World Visuals%%Duration')
    )

    thirdperson
        :AddToggle('Enabled', false, getCallback('Third Person%%Enabled'))
        :AddKeyBind(nil, 'Key Bind')
    thirdperson:AddToggle(
        'Show Character',
        false,
        getCallback('Third Person%%Show Character')
    )
    thirdperson:AddToggle(
        'Show Character While Aiming',
        false,
        getCallback('Third Person%%Show Character While Aiming')
    )
    thirdperson:AddSlider(
        ...
        1,
        ' Studs',
        getCallback('Third Person%%Camera Offset X')
    )
    thirdperson:AddSlider(
        'Camera Offset Y',
        0,
        ...
        1,
        ' Studs',
        getCallback('Third Person%%Camera Offset Y')
    )
    thirdperson:AddSlider(
        'Camera Offset Z',
        7,
        ...
        1,
        ' Studs',
        getCallback('Third Person%%Camera Offset Z')
    )
    thirdperson:AddToggle(
        'Camera Offset Always Visible',
        true,
        getCallback('Third Person%%Camera Offset Always Visible')
    )
    thirdperson:AddToggle(
        'Apply Anti Aim To Character',
        true,
        getCallback('Third Person%%Apply Anti Aim To Character')
    )
    ...
    customChar
        :AddToggle(
            ...
            false,
            getCallback('Custom Model%%Custom Character Model')
        )
        :AddKeyBind(nil, 'Key Bind')
    customChar:AddTextBox(
        'Asset ID',
        'ID',
        getCallback('Custom Model%%Asset ID')
    )
    customChar:AddSlider(
        'Asset Offset X',
        0,
        -10,
        10,
        0.2,
        ' Studs',
        getCallback('Custom Model%%Asset Offset X')
    )
    customChar:AddSlider(
        'Asset Offset Y',
        0,
        ...
        0.2,
        ' Studs',
        getCallback('Custom Model%%Asset Offset Y')
    )
    customChar:AddSlider(
        'Asset Offset Z',
        0,
        ...
        0.2,
        ' Studs',
        getCallback('Custom Model%%Asset Offset Z')
    )

    crosshair
        :AddToggle('Enabled', false, getCallback('Crosshair%%Enabled'))
        :AddKeyBind(nil, 'Key Bind')
        :AddColorPicker(
            'Crosshair Color',
            Color3.new(0.1, 0.1, 1),
            getCallback('Crosshair%%Crosshair Color')
        )
    crosshair:AddToggle('Show Dot', false, getCallback('Crosshair%%Show Dot'))
    crosshair:AddToggle(
        'Follow Recoil',
        false,
        getCallback('Crosshair%%Follow Recoil')
    )
    crosshair:AddSlider(
        'X Size',
        10,
    ...
    crosshair:AddSlider(
        ...
        0.05,
        ' Spins/Second',
        getCallback('Crosshair%%Spin Speed')
    )
    crosshair:AddToggle(
        'Rainbow Crosshair',
        false,
        getCallback('Crosshair%%Rainbow Crosshair')
    )
    crosshair:AddSlider(
        'Rainbow Speed',
        0.5,
        ...
        0.05,
        ' Rainbows/Second',
        getCallback('Crosshair%%Rainbow Speed')
    )

    movement
        :AddToggle('Walk Speed', false, getCallback('Movement%%Walk Speed'))
        :AddKeyBind(nil, 'Walk Bind')
    movement:AddSlider(
        'Set Speed',
        50,
        10,
        ...
        1,
        ' Studs/Second',
        getCallback('Movement%%Set Speed')
    )
    movement
        :AddToggle('Jump Power', false, getCallback('Movement%%Jump Power'))
        :AddKeyBind(nil, 'Jump Bind')
    movement:AddSlider(
        'Height Addition',
        10,
        1,
        ...
        1,
        ' Studs',
        getCallback('Movement%%Height Addition')
    )
    movement:AddToggle(
        'No Fall Damage',
        false,
        getCallback('Movement%%No Fall Damage')
    )
    movement
        :AddToggle('Bunny Hop', false, getCallback('Movement%%Bunny Hop'))
        :AddKeyBind(nil, 'BHop Bind')
    movement:AddToggle(
        'Only While Jumping',
        true,
        getCallback('Movement%%Only While Jumping')
    )
    ...
    sounds:AddDropdown(
        ...
        'None',
        soundFileList,
        getCallback('Sounds%%Shoot Sound')
    )
    sounds:AddDropdown(
        'Hit Sound',
        'None',
        soundFileList,
        getCallback('Sounds%%Hit Sound')
    )
    sounds:AddDropdown(
        'Kill Sound',
        'None',
        soundFileList,
        getCallback('Sounds%%Kill Sound')
    )
    sounds:AddDropdown(
        'Got Hit Sound',
        'None',
        soundFileList,
        getCallback('Sounds%%Got Hit Sound')
    )
    sounds:AddDropdown(
        'Glass Breaking Sound',
        'None',
        soundFileList,
        getCallback('Sounds%%Glass Breaking Sound')
    )
    sounds:AddDropdown(
        'Footstep Sound',
        'None',
        soundFileList,
        getCallback('Sounds%%Footstep Sound')
    )

    tweaks:AddToggle(
        'Custom Kill Notification',
        false,
        getCallback('Tweaks%%Custom Kill Notification')
    )
    tweaks:AddTextBox(
        'Notification Text',
        'Furry Killed!',
        getCallback('Tweaks%%Notification Text')
    )
    tweaks:AddButton(
        'Unlock All Attachments',
        getCallback('Tweaks%%Unlock All Attachments')
    )
    tweaks:AddButton(
        'Unlock All Knives',
        getCallback('Tweaks%%Unlock All Knives')
    )
    tweaks:AddButton(
        'Unlock All Camos',
        getCallback('Tweaks%%Unlock All Camos')
    )
    ...
    chatspam
        :AddToggle('Enabled', false, getCallback('Chat Spam%%Enabled'))
        :AddKeyBind(nil, 'Key Bind')
    chatspam:AddDropdown(
        'Spam List',
        'default.txt',
        chatListsFiles,
        getCallback('Chat Spam%%Spam List')
    )
    chatspam:AddSlider(
        'Spam Delay',
        2.51,
        ...
        0.01,
        ' Seconds',
        getCallback('Chat Spam%%Spam Delay')
    )
    ...
    hopper:AddButton(
        'Copy Join Script',
        getCallback('Server Hopper%%Copy Join Script')
    )
    hopper:AddButton(
        'Clear Cached Servers',
        getCallback('Server Hopper%%Clear Cached Servers')
    )
    ...
    settings:CreatePlayerList({ 'Friendly', 'Target' }, {
        ...
        votekick = function(player) end,
        spectate = function(player) end,
    })

    local cheatSettings =
        settings:CreateSection('Cheat Settings', false, 'third')
    local configuration = settings:CreateSection('Configuration', true, 'third')

    cheatSettings:AddToggle(
        'Save Last Config',
        true,
        getCallback('Cheat Settings%%Save Last Config')
    )
    cheatSettings:AddToggle(
        'Show Keybind List',
        false,
        getCallback('Cheat Settings%%Show Keybind List')
    )
    cheatSettings:AddToggle(
        'Show Key Name',
        false,
        getCallback('Cheat Settings%%Show Key Name')
    )
    cheatSettings:AddButton('Copy Discord Invite', function()
        setclipboard('https://discord.gg/bbot')
    end)
    ...
    local configList = configuration:AddDropdown(
        'Config List',
        'Config',
        getConfigNames(),
        getCallback('Configuration%%Config List')
    )
    configuration:AddButton(
        'Load Config',
        getCallback('Configuration%%Load Config')
    )
    configuration:AddButton(
        'Update Config List',
        getCallback('Configuration%%Update Config List')
    )
    configuration:AddTextBox(
        'Config Name',
        'New Config',
        getCallback('Configuration%%Config Name')
    )
    configuration:AddButton(
        'Save Config',
        getCallback('Configuration%%Save Config')
    )
    ...
    callbackList['Cheat Settings%%Show Keybind List'] = function(bool)
        if bool then
            task.delay(0.05, function()
                menu:CreateKeyList(
                    menu:GetValue('Cheat Settings', 'Show Key Name')
                )
            end)
    ...
    callbackList['Configuration%%Load Config'] = function()
        ...
        if
            fileName
            and isfile(folderName .. '/configs/' .. fileName .. '.json')
        then
            loadConfig(
                httpService:JSONDecode(
                    readfile(folderName .. '/configs/' .. fileName .. '.json')
                )
            )
    ...
    callbackList['Configuration%%Save Config'] = function()
        saveConfig(
            folderName
                .. '/configs/'
                .. menu:GetValue('Configuration', 'Config Name')
                .. '.json',
            getConfig()
        )
    end
    ...
    if configExists then
        local config = httpService:JSONDecode(
            readfile(folderName .. '/cache/lastfile.json')
        )
        ...
        if keybinds then
            for _, data in keybinds do
                ...
                menu.sectionIndexes[section].flags[element].keybind:SetValue(
                    key
                )
            end
        end
